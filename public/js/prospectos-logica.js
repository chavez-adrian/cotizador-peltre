...
// se redefine ni se envuelve, solo se decide QUE campos pasan por el.
//
...
export function normalizarTextosProspecto(datos) {
  const d = datos || {};
  const salida = { ...d };
  if (d.nombre !== undefined) salida.nombre = aTitulo(d.nombre);
  if (d.ciudad !== undefined) salida.ciudad = aTitulo(d.ciudad);
  if (d.data) {
    const data = { ...d.data };
    if (data.empresa !== undefined) data.empresa = aTitulo(data.empresa);
