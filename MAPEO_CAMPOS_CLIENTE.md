...
**Versión:** 1.0  
**Fecha:** 2026-06-05  
**Propósito:** Cruzar los campos requeridos por el SOP de alta de clientes en Operam contra lo que el cotizador captura actualmente, lo que llega a la API de Operam y lo que queda registrado en Neon DB. El resultado de este análisis es el insumo principal para el PRD pendiente de mejoras al flujo de clientes.
...
| Cuenta de ventas | Cuenta de Ventas | Domicilio/Contabilidad > Cuenta de Ventas | 43–44 | — | — | — | — | **GAP** — no en la API; valores: `401-01-001` (México) / `401-07-000` (exportación) |
| Almacén predeterminado | Almacén de Inventario Predeterminada | Domicilio > Almacén | 21–22 | — | — | — | — | **GAP** — SOP requiere `PT`; no automatizado |
...
| Clasificación del contacto | Contacto para | Contacto > Contacto para | 34–35 | — | — | — | — | **GAP** — SOP exige General / Invoices / Deliveries; no existe en ningún formulario |
...
| Referencias de entrega | Referencias | Domicilios | 42 | — | `cl-referencias` | — | — | PARCIAL — ausente en csf-upload.html; no se envía a Operam |
...
| 2 | Cuenta de ventas | "Cuenta de Ventas" | **Crítico** | El SOP (paso 43–44) requiere elegir `401-01-001` para México o `401-07-000` para exportación. No está en la API. Se hace manualmente después del alta. |
...
| 4 | Almacén predeterminado | "Almacén de Inventario Predeterminada" | **Alto** | El SOP (paso 21–22) requiere seleccionar `PT` (producto terminado). No está en la API. Se configura manualmente. |
...
| 7 | Referencias de entrega | "Referencias" | **Medio** | `csf-upload.html` no tiene campo de referencias de entrega. El cotizador principal sí tiene `cl-referencias` pero no se envía a Operam. |
...
| Área / Zona de venta | Área/Zona de Venta | `area` | `1` | `10` para México; `20` USA; `30` Canadá | Sí — todos los clientes quedan con área `1` independientemente |
| Ubicación | (interno) | `location` | `'40'` | No especificado en SOP | Sin evidencia de problema por ahora |
...
Este mapeo es el insumo directo para el PRD de mejoras al cotizador. Las decisiones que el PRD deberá tomar incluyen:

- Qué gaps son prioritarios de resolver en una primera iteración.
- Si algunos campos hardcodeados deben volverse configurables o si el default es siempre correcto para Peltre Nacional.
- Si el flujo de alta desde `csf-upload.html` y el flujo desde el cotizador principal deben unificarse o mantenerse separados.
- Si la tabla `clientes_log` en Neon debe ampliarse para auditoría más completa.

Ver `PROCESO_COMERCIAL_AS_IS.md` para el contexto del proceso comercial completo donde aplica el alta de clientes.
