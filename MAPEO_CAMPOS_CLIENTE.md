...
| Nombre corto | (cust_ref interno) | Conf. General | — | `csf-nombre-corto` / `manual-nombre-corto` — se lee (`altaCsfLeerFormulario`, `app.js:3905`) pero se descarta | POST genérico → `cust_ref` solo si viene de `c.nombreCorto` de la cotización (`lib/alta-generica.js:59`); **el upgrade NUNCA lo envía** — `cust_ref` no está en `DIFF_FISCAL_CAMPOS` (`alta-logica.js:108-120`) | — | **PARCIAL (regresión nueva)** — capturado en el formulario de upgrade y tirado a la basura; si el nombre corto llega junto con la CSF (no en la cotización original) nunca se actualiza en Operam |
...
| Tax ID extranjero | — | — | — | **No existe ningún input en `index.html`/`app.js`** (grep sin resultados) | `buildClienteBody` sigue leyendo `cliente.invoice_tax_id` para prefijo de `notes` (`operam-client.js:312`), pero nada lo puebla | — | **GAP (regresión)** — antes era PARCIAL (se concatenaba a notas desde `csf-upload.html`); hoy no se captura en ningún lado, el código muerto en `operam-client.js` es la única referencia |

### 2.2 Dirección fiscal

| Campo semántico | Label en Operam | Ruta en Operam | SOP paso | Captura en UI hoy | API Operam | Neon | Estado |
|---|---|---|---|---|---|---|---|
| Calle | Calle | Nombre y Dirección | 5 | `csf-calle` (tab CSF); **ausente en tab "Captura manual"** (`altaManualLeerFormulario`, `app.js:3998-4012`, sin `calle`) | PUT upgrade → `street` (`DIFF_FISCAL_CAMPOS`) solo si viene de la tab CSF | — | **PARCIAL** — resuelto para la ruta CSF (antes PARCIAL "solo en csf-upload", ahora sí llega vía upgrade); **GAP nuevo en la ruta manual**: `altaManualConfirmar` nunca pregunta la calle, así que un upgrade manual no puede setear domicilio fiscal |
| Número exterior | Número | Nombre y Dirección | 5 | `csf-num-ext`; ausente en manual | PUT upgrade → `street_number` | — | PARCIAL — mismo patrón que Calle |
| Número interior | Interior | Nombre y Dirección | 5 | `csf-num-int`; ausente en manual | PUT upgrade → `suite_number` | — | PARCIAL — mismo patrón |
| Colonia | Colonia | Nombre y Dirección | 5 | `csf-colonia`; ausente en manual | PUT upgrade → `district` | — | PARCIAL — mismo patrón |
| Código postal fiscal | C.P. | Nombre y Dirección | 5 | `csf-cp` y `manual-cp` (ambas tabs) | PUT upgrade → `postal_code` | — | OK |
| Municipio | Ciudad / Municipio | Nombre y Dirección | 5 | `csf-municipio` y `manual-municipio` | PUT upgrade → `city` | — | OK |
| Estado | Estado | Nombre y Dirección | 5 | `csf-estado` y `manual-estado` | PUT upgrade → `state` | — | OK |
...
| Uso de CFDI | Uso de CFDI | CFDI | — | `csf-uso-cfdi` / `manual-uso-cfdi` (selector, se lee en `altaCsfLeerFormulario`/`altaManualLeerFormulario`) | **No está en `DIFF_FISCAL_CAMPOS`** — el upgrade nunca lo envía; solo se usa en `buildAltaDarDeAltaPayload` (`alta-logica.js:488`), que alimenta el `POST /api/crear-cliente` del acordeón completo, **inalcanzable desde la UI** (ver hallazgo estructural) | — | **GAP (regresión) o dato muerto** — el selector existe, el vendedor lo llena, y no llega a ningún lado en el camino que sí puede ejecutarse (upgrade). `buildClienteBody` sigue defaulteando `timbrado_uso_cfdi` a `'S01'` siempre (`operam-client.js:334`) |
...
| Segmento de cliente | Segmento Cliente | Conf. General > Segmento Cliente | 8–9 | No se captura en ningún formulario del flujo vivo | `buildClienteGenerico` (`lib/alta-generica.js:51-65`) **no incluye `segmento_id`**; el único código que lo setea es `buildAltaDarDeAltaPayload` del acordeón completo, inalcanzable | — | **GAP (persiste)** — sigue sin llegar a Operam en el único camino que hoy crea clientes |
...
| Email para factura | — | — | — | `cl-email-factura` (paso Cliente) / `invoice_email` en `buildAltaDarDeAltaPayload` (acordeón completo, inalcanzable) | La alta genérica (`buildClienteGenerico`) **no incluye `invoice_email`** en absoluto | — | **PARCIAL (regresión)** — antes OK; hoy se captura en `cl-email-factura` pero `lib/alta-generica.js` no lo lee ni lo manda — solo llegaría vía el acordeón completo, hoy inalcanzable |
...
| **11 (nuevo)** | Nombre corto (`cust_ref`) no viaja en el upgrade fiscal | Medio | `DIFF_FISCAL_CAMPOS` no incluye `cust_ref`; el vendedor lo captura en `csf-nombre-corto`/`manual-nombre-corto` y se descarta silenciosamente al confirmar un upgrade. |
| **12 (nuevo)** | Tax ID extranjero: ya no se captura en absoluto | Bajo (uso raro, clientes extranjeros) | Antes PARCIAL (se concatenaba a notas desde `csf-upload.html`); hoy ningún input existe. `operam-client.js:312` conserva la lectura de `cliente.invoice_tax_id` como código muerto. |
| **13 (nuevo)** | Uso de CFDI: capturado y descartado | Medio | El selector (`csf-uso-cfdi`/`manual-uso-cfdi`) se llena y solo se usa en el payload del acordeón inalcanzable (gap #9); `buildClienteBody` sigue defaulteando `'S01'` siempre en la alta genérica. |
| **14 (nuevo)** | Email de facturación no llega en la alta genérica | Medio | `cl-email-factura` se captura pero `buildClienteGenerico` no lee `invoice_email`; solo llegaría vía el acordeón inalcanzable. |
| **15 (nuevo)** | Dirección fiscal (calle/num.ext/num.int/colonia) y país: ausentes en la tab "Captura manual" del upgrade | Medio | Un upgrade hecho sin PDF (tab manual) no puede setear domicilio fiscal completo — la tab solo pide CP/municipio/estado, no calle ni país en el `DIFF_FISCAL_CAMPOS`. |
| **16 (nuevo)** | Segmento de cliente | Alto (heredado de auditoría anterior, sigue sin resolverse) | `buildClienteGenerico` no incluye `segmento_id`; sigue sin capturarse en ningún camino vivo. |
...
|---|---|---|---|---|---|
| Uso de CFDI | Uso de CFDI | `timbrado_uso_cfdi` | `'S01'` | Configurable; verificar con el cliente antes de facturar | Sí — el selector existe en la UI (`csf-uso-cfdi`/`manual-uso-cfdi`) pero se ignora en el único camino vivo (gap #13) |
