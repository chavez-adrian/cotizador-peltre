...
//
// `segmentos` (el catalogo de Operam, que vive en server.js) y `operamUrl` entran
// como datos para que la fila salga lista para pintar: el id del segmento con su
// nombre y la liga "Ver en Operam" a la ficha del cliente. Sin catalogo se
// conserva el id, que es mejor que una celda vacia.
export function filasSegmentoPendiente(rows, { segmentos = [], operamUrl = '' } = {}) {
  const log = rows || [];
  const nombreDelSegmento = id => segmentos.find(s => String(s.id) === String(id))?.nombre || String(id ?? '');
  const base = String(operamUrl || '').replace(/\/+$/, '');
  ...
  return [...porCliente.values()]
    ...
    .map(r => ({
      ...
      nombre: r.nombre || '',
      rfc: r.rfc || '',
      segmento: nombreDelSegmento(segmentoDeFuente(r.fuente)),
      ...capasDelMotivo(r.error_msg),
      created_at: new Date(r.created_at).toISOString(),
      url: `${base}/sales/manage/customers.php?debtor_no=${r.cliente_id}`,
    }));
}
