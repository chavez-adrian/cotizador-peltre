...
import { oportunidadesQueFaltaCotizar } from './oportunidades.js';
...
export function calcularColaHoy(prospectos, cotizaciones, ahora = new Date()) {
  // #400: la etiqueta "Ya tiene Cliente Operam, falta cotizar" necesita las
  // cotizaciones, y la cola del dia ya recibe las que quien pregunta puede ver:
  // se juzga aqui, con el mismo armador de la lista de Prospectos, y el motor de
  // la cadencia solo transporta el resultado.
  const colaProspectos = calcularColaProspectos(
    prospectos, ahora, oportunidadesQueFaltaCotizar(prospectos, cotizaciones)
  )
