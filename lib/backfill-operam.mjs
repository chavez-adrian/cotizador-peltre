...
// Clientes GENERICOS (decision Adrian 2026-07-29/2026-07-30, #76): sus cotizaciones NO
// se importan al pipeline. No son oportunidades de mayoreo con una contraparte
// nombrada, sino ventas agrupadas bajo un RFC generico; se difieren al issue #118, que
// las rescatara como PROSPECTOS (con el contactoEntrega y el customer_ref que el quote
// SI trae). Verificados en vivo contra la API el 2026-07-30: 143 (GENERICO TIENDAS
// FISICAS USD), 183 (GENERICO TIENDAS FISICAS), 184 (GENERICO TIENDAS DIGITALES), 256
// (GENERICO DIGITAL USD), 449 (GENERICO EXPORTACION). Se exporta para que #118 reuse el
// mismo set en vez de repetir los ids sueltos.
...
// DEBTORS_SOCIOS, no se duplican aqui.) Aplica en AMBAS partes: la B (id-walk, donde
// se detectaron) y la A (por si alguno se convirtio en pedido despues).
export const FOLIOS_EXCLUIDOS_MANUAL = new Set(['1189', '1195', '1196']);
...
export async function planearBackfill(deps = {}) {
  ...
  for (const pedido of pedidos) {
    ...
    const debtorNo = strOrNull(pedido.debtor_no);
    if (debtorNo != null && DEBTORS_GENERICOS.has(debtorNo)) { plan.skips.generico++; continue; }
...
// banda es ANCHA a proposito (75% relativo al monto de la COTIZACION): autorizar 100 de
// las 250 piezas cotizadas sigue siendo la misma compra. El limite inferior lo fija el
// caso MOTOBLOCK (revision en vivo 2026-07-30): un pedido de muestras de $598 contra una
// cotizacion de $182,093 (0.3%) NO es la misma compra -- esa cotizacion DEBE importarse.
// De ahi que la comparacion sea relativa al monto de la cotizacion y no al del pedido.
//
...
export function pedidoQueCierra(quote, pedidosDelCliente, opts = {}) {
  ...
  for (const p of pedidosDelCliente || []) {
    ...
    if (Math.abs(totalPedido - totalQuote) / totalQuote > banda) continue;
    return p;
  }
  return null;
}
