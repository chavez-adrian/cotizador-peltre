...
// Reusa estadoPago (regla de pago del nucleo #62), no la reimplementa.
export function esCerrado(hechos) {
  return !!(hechos && hechos.tieneRemision && estadoPago(hechos.pago) === 'pagado');
}

// CRITERIO 2 #76: la etapa de la tarjeta para el backfill. Para el entregado-IMPAGO
// (tiene remision pero NO esta pagado al 100%) se deriva la etapa IGNORANDO la
// remision (tieneRemision: false), para que la tarjeta refleje el AVANCE DE PAGO
// (pedido_liberado/anticipo_pagado) y NO se vea como producto_entregado (cerrada),
// que ocultaria la cobranza pendiente. En cualquier otro caso (no entregado, o
// entregado y pagado -- este ultimo ya lo filtra esCerrado antes de llegar aqui) se
// usa la etapa post-venta normal. null de etapaPostVenta (sin hecho post-venta) cae
// a 'seguimiento' (la cotizacion existe pero sin senal de pago/entrega todavia).
export function etapaBackfill(hechos, op) {
  if (hechos && hechos.tieneRemision && estadoPago(hechos.pago) !== 'pagado') {
    return etapaPostVenta({ ...hechos, tieneRemision: false }, op) || 'seguimiento';
  }
...
export async function planearBackfill(deps = {}) {
  ...
  for (const pedido of pedidos) {
    ...
    if (esCerrado(hechos)) { plan.skips.cerrado++; continue; }
    const etapa = etapaBackfill(hechos, opParaEtapa);
