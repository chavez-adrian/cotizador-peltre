...
// Cliente GENERICO de las tiendas digitales (decision Adrian 2026-07-29, #76): sus
// cotizaciones NO se importan al pipeline. No son oportunidades de mayoreo con una
// contraparte nombrada, sino ventas digitales sueltas agrupadas bajo un RFC generico;
// se difieren al issue #118, que las rescatara como PROSPECTOS (con el contactoEntrega
// y el customer_ref que el quote SI trae). Se exporta para que #118 reuse el mismo id
// en vez de repetir el numero suelto.
export const DEBTOR_GENERICO_TIENDAS_DIGITALES = '184';
...
export async function planearBackfill(deps = {}) {
  ...
  const plan = {
    ...
    skips: { noCandidato: 0, otraSucursal: 0, generico: 0, cerrado: 0, cancelado: 0, duplicado: 0 },
  };
  ...
  for (const pedido of pedidos) {
    ...
    // GENERICO TIENDAS DIGITALES: fuera del backfill, diferido a #118 (prospectos).
    // Va DESPUES de registrar el folio en foliosConPedido, por lo mismo que el cancelado.
    if (strOrNull(pedido.debtor_no) === DEBTOR_GENERICO_TIENDAS_DIGITALES) { plan.skips.generico++; continue; }
...
export async function planearBackfillSinPedido(deps = {}) {
  ...
  const plan = {
    ...
    importar: [],
    skips: { conPedido: 0, otraSucursal: 0, generico: 0, prueba: 0, cancelado: 0, duplicado: 0, sinDebtor: 0 },
  };
  ...
  for (let folio = Number(folioMax); folio >= folioMin; folio--) {
    ...
    // GENERICO TIENDAS DIGITALES: fuera del backfill, diferido a #118 (prospectos). Es
    // el grueso de la parte B, de ahi el contador propio.
    if (debtorNo === DEBTOR_GENERICO_TIENDAS_DIGITALES) { plan.skips.generico++; continue; }
