...
export function serializarBodyQuote(campos, { deliveryDate, comments, custRef, deliverTo, deliveryAddress } = {}) {
  ...
  // vacio. Desde #328 son cuatro (Comments, cust_ref, deliver_to, delivery_address) y la
  // regla vive una sola vez.
...
export async function actualizarQuoteOperam(quoteNo, data) {
  const { items, comments, custRef, vigencia, deliverTo, deliveryAddress } = armarContenidoQuote(data || {});
  ...
  return enCola(async () => {
    ...
    try {
      ...
      const body = serializarBodyQuote(campos, { deliveryDate: vigencia, comments, custRef, deliverTo, deliveryAddress });
      escrito = true;
