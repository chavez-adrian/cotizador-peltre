...
export function opcionesListaQuote(html) {
  const texto = String(html ?? '').replace(/<!--[\s\S]*?-->/g, '');
  const select = texto.match(/<select[^>]*name\s*=\s*(?:"sales_type"|'sales_type')[^>]*>([\s\S]*?)<\/select>/i);
  if (!select) return [];
...
export function serializarBodyQuote(campos, { deliveryDate, comments, custRef, deliverTo, deliveryAddress, contactPhone, contactEmail, salesType } = {}) {
