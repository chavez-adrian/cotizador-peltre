...
export function serializarBodyQuote(campos, { deliveryDate, comments, custRef, deliverTo, deliveryAddress, contactPhone, contactEmail, salesType, branchId } = {}) {
...
async function verificarListaEscrita(quoteNo, esperado, leerLista) {
  let leido;
  try {
    ...
    return {
      ...
      motivo: 'no se pudo releer la lista del encabezado en Operam: ' + err.message,
    };
  }
...
export async function corregirVigenciaQuote(quoteNo, vigencia, { lista = null, leerLista = leerListaDelQuote } = {}) {
  return enCola(async () => {
    ...
    if (campos.delivery_date === vigencia && !decision.escribir) {
      return {
...
export async function actualizarQuoteOperam(quoteNo, data, { lista = null, leerLista = leerListaDelQuote } = {}) {
  ...
  if (!items.length) {
    return { ok: false, escrito: false, verificado: false, discrepancias: [], lista: listaNoEscrita('la actualizacion se rechazo antes de abrir la sesion de edicion'), error: 'La cotizacion no tiene partidas: no se actualiza el quote de Operam para no dejarlo vacio' };
  }
  return enCola(async () => {
    let escrito = false;
    try {
      ...
      return { ok: false, escrito, verificado: false, discrepancias: [], lista: listaNoEscrita('la actualizacion del quote fallo antes de poder verificar la lista'), error: err.message };
    }
  });
}
