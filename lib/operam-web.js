...
export async function abrirSesionWeb({ base = process.env.OPERAM_URL, user = process.env.OPERAM_USER, pass = process.env.OPERAM_PASSWORD } = {}) {
  ...
  // GET inicial para sembrar la cookie de sesion, luego POST del login a la misma pagina
  // (FA responde 303 a la misma URL ya autenticado).
  ...
  return async function consultar(transNo, transType) {
    // Retry ante caidas de red transitorias (ECONNRESET): el detector hace cientos de
    // consultas; una caida puntual no debe abortar la corrida.
    for (let intento = 0; ; intento++) {
      try {
        const res = await fetch(pageUrl(transNo, transType), { headers: { Cookie: cookie() }, redirect: 'manual' });
        return res.text();
      } catch (err) {
        if (intento < 4) { await sleep(1000 * Math.pow(2, intento)); continue; }
        throw err;
      }
    }
  };
