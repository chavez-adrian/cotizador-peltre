...
export function serializarBodyQuote(campos, { deliveryDate, comments, custRef, deliverTo, deliveryAddress } = {}) {
  ...
  // (declarado con comillas SIMPLES en el HTML de FA; extraerCampos ya lo recoge). Misma
  // guarda que los de arriba: si el campo no esta, la pagina no es la que se esperaba y
  // se aborta antes de escribir en vez de perder el dato en silencio.
  if (deliverTo !== undefined) {
    if (!('deliver_to' in campos)) {
      throw new Error('El formulario de Operam no trae deliver_to: la pagina no es la de edicion de cotizacion que se esperaba');
    }
    sustituciones.deliver_to = deliverTo;
  }
  if (deliveryAddress !== undefined) {
    if (!('delivery_address' in campos)) {
      throw new Error('El formulario de Operam no trae delivery_address: la pagina no es la de edicion de cotizacion que se esperaba');
    }
    sustituciones.delivery_address = deliveryAddress;
  }
