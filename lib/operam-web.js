...
import { armarContenidoQuote, obtenerQuote } from './operam-client.js';
...
// El formulario de captura del quote. Abrirlo arranca la edicion en $_SESSION: por eso
// lo comparten los tres caminos que reposteen el documento, en vez de rearmar la URL.
const urlEdicionQuote = (base, quoteNo) =>
  `${base}/sales/sales_order_entry.php?ModifyQuotationNumber=${encodeURIComponent(quoteNo)}`;
...
// El POST de la API v3 ignora la lista por cualquier nombre (12 llaves medidas en
// vivo): el quote nace con la lista del CLIENTE aunque se haya cotizado en otra, y
// el pedido que se derive hereda ESE encabezado. La web legacy si la escribe, en el
// mismo ProcessOrder que ya corrige la vigencia, y NO re-precia las partidas
// (verificado sobre el quote 1287, 2026-09-20).

// Las listas que el formulario de edicion ofrece, `{ id, nombre }` en su orden. Se
// leen APARTE de parsearFormularioQuote -- que solo devuelve el valor seleccionado --
// porque escribir una lista que el formulario no ofrece es pedirle a FA que rechace
// el POST entero, con la vigencia adentro. Lista vacia = el select no esta (la pagina
// no es la que creemos), y entonces no se escribe nada.
// Las opciones que un <select> del formulario ofrece, `{ id, nombre }` en su orden.
// Lo comparten los DOS campos del encabezado que el cotizador escribe por aqui -- la
// lista de precios (#403) y el domicilio de entrega (#409) -- porque el riesgo es el
// mismo y la lectura tambien: FA rechaza el POST entero si el valor no esta entre las
// opciones, asi que ninguno de los dos se escribe a ciegas.
// El nombre legible de una opcion por su valor. null cuando no se puede resolver: el
// llamador no inventa un nombre ni se lo inventa el id crudo.
function nombreDeOpcion(opciones, valor) {
  if (valor === undefined || valor === null || valor === '') return null;
  const o = (opciones || []).find((x) => String(x.id) === String(valor));
  return o ? o.nombre || null : null;
}

function opcionesDeSelect(html, nombre) {
  const texto = String(html ?? '').replace(/<!--[\s\S]*?-->/g, '');
  const select = texto.match(new RegExp(`<select[^>]*name\\s*=\\s*(?:"${nombre}"|'${nombre}')[^>]*>([\\s\\S]*?)</select>`, 'i'));
  if (!select) return [];
  return [...select[1].matchAll(/<option[^>]*>([^<]*)/gi)].map((m) => ({
    id: decodificar(atributo(m[0], 'value') ?? ''),
    nombre: decodificar(m[1]).trim(),
  }));
}

export function opcionesListaQuote(html) {
  return opcionesDeSelect(html, 'sales_type');
}

// Decide si el ProcessOrder lleva la lista del encabezado. Devuelve VALORES, nunca
// escribe: los dos caminos que reposteran el quote (el post-fix de la vigencia al
// crear y la actualizacion de #104) comparten esta decision. Abstenerse NO es un
// fallo -- el quote ya existe con sus precios correctos y lo unico que queda mal es
// el encabezado --, asi que cada abstencion sale con su `motivo` tecnico para que el
// llamador lo reporte como paso en vez de tumbar la subida.
export function decidirListaQuote({ esperado, actual, opciones = [] } = {}) {
  const lista = String(esperado ?? '').trim();
  if (!lista) return { escribir: false, motivo: 'la cotizacion no tiene una lista de precios resoluble en el catalogo' };
  if (actual === undefined || actual === null) {
    return { escribir: false, motivo: 'el formulario de Operam no trae sales_type: la pagina no es la de edicion de cotizacion que se esperaba' };
  }
  if (!opciones.some((o) => String(o.id) === lista)) {
    const ofrecidas = opciones.map((o) => o.id).join(', ') || '(ninguna)';
    return { escribir: false, motivo: `la lista ${lista} no esta entre las opciones del formulario de Operam (${ofrecidas})` };
  }
  if (String(actual) === lista) return { escribir: false, yaCorrecto: true, motivo: `el quote ya esta en la lista ${lista}` };
  return { escribir: true, salesType: lista };
}

// --- El DOMICILIO DE ENTREGA del encabezado del quote (#409) ------------------
// Gemelo del de arriba, y por la misma razon. Al ACTUALIZAR una cotizacion (#104)
// el header se reposteaba con `branch_id` tal como venia del formulario, asi que
// cambiar de domicilio movia la direccion de texto (`delivery_address`, #328) pero
// NUNCA el domicilio que el quote tiene SELECCIONADO: el documento decia Pestalozzi
// y Operam seguia en Bosques de Europa (medido sobre la 1288, 2026-09-21). El
// `branch_id` es lo que identifica al domicilio de entrega (#252) y lo que hereda el
// pedido que se derive, asi que esa divergencia sobrevive al documento.
//
// El valor del <option> ES el branch_code, el mismo que la cotizacion guarda como
// `cliente.branchId` desde este ticket. Se decide aparte y se reporta con motivo:
// abstenerse NO tumba la actualizacion -- las partidas y la vigencia ya quedaron
// bien y lo unico pendiente es el encabezado.
export function decidirBranchQuote({ esperado, actual, opciones = [] } = {}) {
  const branch = String(esperado ?? '').trim();
  if (!branch) return { escribir: false, motivo: 'la cotizacion no tiene un domicilio de entrega elegido' };
  if (actual === undefined || actual === null) {
    return { escribir: false, motivo: 'el formulario de Operam no trae branch_id: la pagina no es la de edicion de cotizacion que se esperaba' };
  }
  if (!opciones.some((o) => String(o.id) === branch)) {
    const ofrecidas = opciones.map((o) => o.id).join(', ') || '(ninguna)';
    return { escribir: false, motivo: `el domicilio ${branch} no esta entre los del cliente en el formulario de Operam (${ofrecidas})` };
  }
  if (String(actual) === branch) return { escribir: false, yaCorrecto: true, motivo: `el quote ya esta en el domicilio ${branch}` };
  return { escribir: true, branchId: branch };
}

export function opcionesBranchQuote(html) {
  return opcionesDeSelect(html, 'branch_id');
}

// --- El TRANSPORTISTA del encabezado del quote (#448) -------------------------
// El tercer gemelo. El POST de la API v3 no manda transportista y el quote hereda el
// `default_ship_via` del domicilio (1 = "Default" en todos los branches que crea el
// cotizador); el pedido derivado hereda ese encabezado. Viaja en el mismo
// ProcessOrder que ya escribe vigencia, lista y domicilio, con la misma regla: solo
// un id que el formulario OFRECE ("Compania de Transporte", select `ship_via`), porque
// uno ajeno haria que FA rechazara el POST entero. `esperado` es el `shipVia` de la
// linea de transporte elegida (transportistaDeEnvio); null = no se manda nada.
export function decidirTransportistaQuote({ esperado, actual, opciones = [] } = {}) {
  const shipVia = String(esperado ?? '').trim();
  if (!shipVia) return { escribir: false, motivo: 'la cotizacion no tiene un transportista de Operam que mandar' };
  if (actual === undefined || actual === null) {
    return { escribir: false, motivo: 'el formulario de Operam no trae ship_via: la pagina no es la de edicion de cotizacion que se esperaba' };
  }
  if (!opciones.some((o) => String(o.id) === shipVia)) {
    const ofrecidas = opciones.map((o) => o.id).join(', ') || '(ninguna)';
    return { escribir: false, motivo: `el transportista ${shipVia} no esta entre las opciones del formulario de Operam (${ofrecidas})` };
  }
  if (String(actual) === shipVia) return { escribir: false, yaCorrecto: true, motivo: `el quote ya tiene el transportista ${shipVia}` };
  return { escribir: true, shipVia };
}

export function opcionesTransportistaQuote(html) {
  return opcionesDeSelect(html, 'ship_via');
}
...
export function serializarBodyQuote(campos, { deliveryDate, comments, custRef, deliverTo, deliveryAddress, contactPhone, contactEmail, salesType, branchId, shipVia } = {}) {
  ...
  // Cada campo opcional se sustituye SOLO si el caller lo mando, y siempre exigiendo que
  // el formulario ya lo trajera: si no esta, la pagina no es la de edicion que creemos y
  // se aborta antes de escribir, en vez de postear el documento con un campo inventado o
  // vacio. Desde #329 son seis (Comments, cust_ref, deliver_to, delivery_address, phone,
  // email) y la regla vive una sola vez.
  const sustituciones = { delivery_date: deliveryDate };
  const sustituirSiViene = (llave, valor) => {
    if (valor === undefined) return;
    if (!(llave in campos)) {
      throw new Error(`El formulario de Operam no trae ${llave}: la pagina no es la de edicion de cotizacion que se esperaba`);
    }
    sustituciones[llave] = valor;
  };
  sustituirSiViene('Comments', comments);
  sustituirSiViene('cust_ref', custRef);
  // Domicilio de entrega (#328): deliver_to es un input y delivery_address un textarea
  // declarado con comillas SIMPLES en el HTML de FA (extraerCampos ya recoge las dos).
  sustituirSiViene('deliver_to', deliverTo);
  sustituirSiViene('delivery_address', deliveryAddress);
  // Contacto de entrega (#329). Los nombres del formulario legacy NO son los de la API:
  // lo que la API v3 llama contact_phone / contact_email, FA lo expone como `phone` y
  // `email` (verificado en el fixture real del formulario del quote 1216). Una cadena
  // vacia SI se sustituye: borrar el contacto que Operam habia heredado del cliente es
  // parte del arreglo, no un caso a saltar.
  sustituirSiViene('phone', contactPhone);
  sustituirSiViene('email', contactEmail);
  // La lista de precios del encabezado (#403). Solo viene cuando decidirListaQuote
  // resolvio que hay que escribirla: sin ella el body sale con la que el formulario
  // ya traia, exactamente como antes.
  sustituirSiViene('sales_type', salesType);
  // El domicilio de entrega del encabezado (#409). Mismo trato que la lista: solo
  // viene cuando decidirBranchQuote resolvio escribirlo, y sin el el body sale con
  // el que el formulario ya traia -- que es como el quote se quedaba en el domicilio
  // viejo mientras el documento imprimia el nuevo.
  sustituirSiViene('branch_id', branchId);
  // El transportista del encabezado (#448). Mismo trato: solo viene cuando
  // decidirTransportistaQuote resolvio escribirlo.
  sustituirSiViene('ship_via', shipVia);
...
// El ALMACEN del que se entrega ("Entregar de ubicacion" en la vista read-only), por
// NOMBRE -- "PT", "Almacen MP" --, que es como el vendedor lo reconoce; el loc_code
// no le dice nada. Se lee de la MISMA vista que ya se pide para verificar, sin una
// peticion de mas y sin reabrir la sesion de edicion.
//
// Existe por una consecuencia no prevista de #409, encontrada por Adrian en la 1288:
// FA toma el almacen del `default_location` del DOMICILIO, asi que escribir el
// domicilio que el vendedor eligio movio tambien el almacen -- y el domicilio estaba
// mal configurado en Operam (Almacen MP en vez de PT, el de producto terminado). El
// pedido que se derive hereda ese almacen. Es dato del ERP y el cotizador no lo
// corrige, pero callarlo es la misma falla silenciosa que #409 vino a matar.
// null = la vista no trae el campo; el llamador lo trata como "no se pudo saber",
// nunca como "no cambio".
// El paso "almacen" del reporte de la actualizacion. PURO. Solo declara `cambio` con
// las dos puntas conocidas: si alguna no se pudo leer sale `cambio: false` con motivo,
// porque afirmar que NO cambio sin haberlo medido es justo la mentira tranquilizadora
// que este aviso existe para evitar.
export function cambioDeAlmacen(antes, despues) {
  const a = String(antes ?? '').trim();
  const b = String(despues ?? '').trim();
  if (!a || !b) {
    return { cambio: false, de: a || null, a: b || null, motivo: 'no se pudo leer el almacen antes o despues de escribir' };
  }
  if (a === b) return { cambio: false, de: a, a: b };
  return { cambio: true, de: a, a: b };
}

export function leerAlmacenVista(html) {
  const m = String(html ?? '').match(/Entregar de ubicaci[oó]n\s*<\/td>\s*<td[^>]*>([^<]*)/i);
  return m ? decodificar(m[1]).trim() || null : null;
}
...
// --- Las partidas del quote leidas por la API (#406) -------------------------
// La otra forma de mirar las mismas partidas: `detalles[]` del
// GET /api/v3/sales/quote/:folio. La correccion de un quote historico compara la
// lectura de ANTES contra la de DESPUES por aqui y no contra la vista, porque esa
// lectura ya se hizo para detectar el desfase (order_type) y no hay que volver a
// abrir el formulario de captura de FA solo para comparar.
...
export function partidasDeQuote(quote) {
  return (Array.isArray(quote?.detalles) ? quote.detalles : []).map((d) => ({
    sku: String(d?.stk_code ?? '').trim(),
    cantidad: aNumeroApi(d?.quantity),
    precio: aNumeroApi(d?.unit_price),
    descuento: aNumeroApi(d?.discount_percent ?? 0),
  }));
}

// Compara dos lecturas del MISMO quote. `verificado: false` es "no se pudo comprobar"
// -- un quote sin partidas legibles --, que NO es lo mismo que "quedo igual" y en
// ningun caso es ok (mismo criterio que compararQuoteVista).
export function compararPartidasQuote(antes = [], despues = []) {
  const discrepancias = [];
  if (antes.length !== despues.length) {
    discrepancias.push({ campo: 'partidas', esperado: antes.length, encontrado: despues.length });
  }
  for (let i = 0; i < Math.min(antes.length, despues.length); i++) {
    const a = antes[i];
    const d = despues[i];
    if (a.sku !== d.sku) {
      discrepancias.push({ campo: 'sku', linea: i, esperado: a.sku, encontrado: d.sku });
      continue; // con el SKU cruzado, comparar cantidad/precio de esa linea no informa
    }
    for (const campo of ['cantidad', 'precio', 'descuento']) {
      if (!IGUAL_MONTO(a[campo], d[campo])) {
        discrepancias.push({ campo, linea: i, sku: a.sku, esperado: a[campo], encontrado: d[campo] });
      }
    }
  }
  const verificado = antes.length > 0;
  return { ok: verificado && discrepancias.length === 0, verificado, discrepancias };
}

// Lo que se reporta de las partidas cuando no hubo nada que comparar: no se escribio,
// o la relectura no llego. Nunca `ok`.
const partidasSinVerificar = () => ({ ok: false, verificado: false, discrepancias: [] });
...
// La lista del encabezado tal como quedo EN OPERAM. Va por la API v3 (`order_type`
// del GET /api/v3/sales/quote/:id) y no por la web: releer el formulario de edicion
// abriria otra sesion de captura en FA, y la vista read-only no muestra el campo.
// Inyectable para que los tests de orquestacion no dependan de la API (#403).
async function leerListaDelQuote(quoteNo) {
  const q = await obtenerQuote(quoteNo);
  const v = q?.order_type;
  return v == null || v === '' ? null : String(v);
}

// El transportista del encabezado tal como quedo EN OPERAM (#448): `ship_via` del
// mismo GET /api/v3/sales/quote/:id, por la misma razon que la lista. Inyectable.
async function leerTransportistaDelQuote(quoteNo) {
  const q = await obtenerQuote(quoteNo);
  const v = q?.ship_via;
  return v == null || v === '' ? null : String(v);
}

// El resultado del paso de la lista cuando NO se escribio. `aplica` distingue los dos
// silencios: "no habia lista que escribir" (la cotizacion no resuelve una, y no hay
// nada que avisar) de "si habia y no se pudo" (el motivo dice por que). El quote que
// ya estaba en esa lista cuenta como correcto y verificado: se acaba de leer del
// formulario. Desde #448 sirve igual al transportista: la forma del paso es la misma.
function listaSinEscribir(esperado, decision = {}) {
  const pedida = esperado == null || esperado === '' ? null : String(esperado);
  const yaCorrecto = decision.yaCorrecto === true;
  return {
    aplica: pedida !== null,
    esperado: pedida,
    escrita: false,
    yaCorrecto,
    ok: pedida === null || yaCorrecto,
    verificado: yaCorrecto,
    encontrado: yaCorrecto ? pedida : null,
    motivo: decision.motivo ?? null,
  };
}

// Relectura obligatoria despues de escribir: Operam responde 200 aunque ignore
// campos. verificado=false es "no se pudo comprobar" (la relectura fallo o vino sin
// el campo), que NO es lo mismo que "quedo mal" -- y en ningun caso es exito.
// `que` nombra el campo en el motivo: la lista (#403) o el transportista (#448).
async function verificarListaEscrita(quoteNo, esperado, leerLista, que = 'la lista del encabezado') {
  let leido;
  try {
    leido = await leerLista(quoteNo);
  } catch (err) {
    return {
      aplica: true, esperado, escrita: true, yaCorrecto: false,
      ok: false, verificado: false, encontrado: null,
      motivo: 'no se pudo releer ' + que + ' en Operam: ' + err.message,
    };
  }
  const encontrado = leido == null || leido === '' ? null : String(leido);
  return {
    aplica: true, esperado, escrita: true, yaCorrecto: false,
...
//
// Desde #403 este mismo POST lleva tambien la LISTA DE PRECIOS del encabezado, que es
// el otro campo del quote que el cotizador manda y la API v3 no escribe. Por eso el
// corto circuito ya no depende solo de la vigencia: con la fecha correcta y la lista
// equivocada hay que repostear igual. El resultado de la lista viaja aparte, en
// `lista`, y no altera el veredicto de la vigencia: son dos campos independientes y
// el llamador reporta un paso por cada uno.
//
// Desde #448 lleva tambien el TRANSPORTISTA (`ship_via`) de la linea de transporte
// elegida, con el mismo trato que la lista: entra al corto circuito, viaja aparte en
// `transportista` y no altera el veredicto de la vigencia.
export async function corregirVigenciaQuote(quoteNo, vigencia, {
  lista = null, leerLista = leerListaDelQuote,
  transportista = null, leerTransportista = leerTransportistaDelQuote,
} = {}) {
  return enCola(async () => {
    const s = await sesion();
    const html = await s.pedir(urlEdicionQuote(s.base, quoteNo));
    const { campos, action } = parsearFormularioQuote(html);
    const decision = decidirListaQuote({ esperado: lista, actual: campos.sales_type, opciones: opcionesListaQuote(html) });
    const decisionTransportista = decidirTransportistaQuote({
      esperado: transportista, actual: campos.ship_via, opciones: opcionesTransportistaQuote(html),
    });
    if (campos.delivery_date === vigencia && !decision.escribir && !decisionTransportista.escribir) {
      return {
        ok: true, yaCorrecto: true, verificado: true, esperado: vigencia, encontrado: vigencia,
        lista: listaSinEscribir(lista, decision),
        transportista: listaSinEscribir(transportista, decisionTransportista),
      };
    }
    const body = serializarBodyQuote(campos, {
      deliveryDate: vigencia,
      ...(decision.escribir ? { salesType: decision.salesType } : {}),
      ...(decisionTransportista.escribir ? { shipVia: decisionTransportista.shipVia } : {}),
    });
    ...
    return {
      ...
      esperado: vigencia,
      encontrado,
      lista: decision.escribir
        ? await verificarListaEscrita(quoteNo, decision.salesType, leerLista)
        : listaSinEscribir(lista, decision),
      transportista: decisionTransportista.escribir
        ? await verificarListaEscrita(quoteNo, decisionTransportista.shipVia, leerTransportista, 'el transportista del encabezado')
        : listaSinEscribir(transportista, decisionTransportista),
    };
  });
}

// --- Corregir la lista de un quote HISTORICO (#406) --------------------------
// Los quotes anteriores a #403 quedaron registrados con la lista del CLIENTE (la API
// v3 ignoraba la del encabezado y Operam ponia la de la ficha). Esta operacion corrige
// SOLO ese campo sobre un quote que ya existe: reusa el mismo ProcessOrder de #403 con
// la vigencia TAL COMO el formulario la trae -- mover el "Valido hasta" que el
// vendedor ya acordo con el cliente seria otro cambio, y no se pidio.
//
// Verificacion obligatoria y en dos frentes, porque el ProcessOrder repostea el
// documento entero: (1) el encabezado quedo en la lista pedida, (2) NINGUNA partida
// cambio de SKU, cantidad, precio ni descuento. Las dos lecturas van por la API v3
// (GET /sales/quote/:folio), la misma con la que el inventario detecto el desfase.
//
// Falla CERRADO: un quote cuyas partidas no se pueden leer ANTES de escribir no se
// toca -- no habria contra que verificar despues. Nunca lanza: devuelve
// { ok, escrito, lista, partidas, motivo } y el script decide si sigue o se detiene.
export async function corregirListaQuote(quoteNo, lista, { leerQuote = obtenerQuote } = {}) {
  const esperado = lista == null || lista === '' ? null : String(lista);
  // El quote que NO quedo corregido, con su motivo. `escrito` dice si el documento se
  // llego a tocar: un fallo DESPUES del ProcessOrder no se puede reportar como "no se
  // escribio nada", y es lo que decide si el lote sigue o se detiene.
  const noCorregido = (motivo, escrito = false) => ({
    ok: false, escrito, motivo,
    lista: {
      aplica: esperado !== null, esperado, escrita: escrito, yaCorrecto: false,
      ok: false, verificado: false, encontrado: null, motivo,
    },
    partidas: partidasSinVerificar(),
  });
  if (esperado === null) return noCorregido('no se pidio ninguna lista de precios para el encabezado');

  return enCola(async () => {
    let antes;
    try {
      antes = await leerQuote(quoteNo);
    } catch (err) {
      return noCorregido(`no se pudo leer el quote ${quoteNo} en Operam: ${err.message}`);
    }
    if (!antes) return noCorregido(`Operam no tiene ningun quote con el folio ${quoteNo}`);
    const partidasAntes = partidasDeQuote(antes);
    if (!partidasAntes.length) {
      return noCorregido(`el quote ${quoteNo} no trae partidas legibles: sin ellas no habria contra que verificar lo que se escriba`);
    }
    ...
    try {
      ...
      const html = await s.pedir(urlEdicionQuote(s.base, quoteNo));
      const { campos, action } = parsearFormularioQuote(html);
      const decision = decidirListaQuote({ esperado, actual: campos.sales_type, opciones: opcionesListaQuote(html) });
      if (!decision.escribir) {
        const sinEscribir = listaSinEscribir(esperado, decision);
        return {
          ok: sinEscribir.ok, escrito: false, motivo: decision.motivo ?? null,
          lista: sinEscribir,
          partidas: partidasSinVerificar(),
        };
      }
      // La vigencia viaja EXACTAMENTE como la trajo el formulario: este camino solo
      // corrige la lista.
      const body = serializarBodyQuote(campos, { deliveryDate: campos.delivery_date, salesType: decision.salesType });
      await s.pedir(new URL(action, s.base).href, { method: 'POST', body: body.toString() });
      escrito = true;

      // UNA relectura sirve a las dos verificaciones (el encabezado y las partidas), y
      // se hace aqui -- no dentro del lector que recibe verificarListaEscrita -- para
      // que la comparacion de partidas no dependa de un efecto lateral.
      let despues = null;
      let errorRelectura = null;
      try {
        despues = await leerQuote(quoteNo);
      } catch (err) {
        errorRelectura = err;
      }
      const resultadoLista = await verificarListaEscrita(quoteNo, decision.salesType, () => {
        if (errorRelectura) throw errorRelectura;
        if (!despues) throw new Error(`Operam ya no devuelve el quote ${quoteNo}`);
        return despues.order_type;
      });
      const partidas = despues
        ? compararPartidasQuote(partidasAntes, partidasDeQuote(despues))
        : partidasSinVerificar();
      return {
        ok: resultadoLista.ok && partidas.ok,
        escrito: true,
        motivo: resultadoLista.motivo ?? null,
        lista: resultadoLista,
        partidas,
      };
    } catch (err) {
      return noCorregido(err.message, escrito);
    }
  });
}
...
export async function actualizarQuoteOperam(quoteNo, data, {
  ...
  const { items, comments, custRef, vigencia, deliverTo, deliveryAddress, contactPhone, contactEmail } = armarContenidoQuote(data || {});
  ...
  const listaNoEscrita = (motivo) => listaSinEscribir(lista, { motivo });
  const transportistaNoEscrito = (motivo) => listaSinEscribir(transportista, { motivo });
  if (!items.length) {
    const motivo = 'la actualizacion se rechazo antes de abrir la sesion de edicion';
    return { ok: false, escrito: false, verificado: false, discrepancias: [], lista: listaNoEscrita(motivo), transportista: transportistaNoEscrito(motivo), error: 'La cotizacion no tiene partidas: no se actualiza el quote de Operam para no dejarlo vacio' };
  }
  return enCola(async () => {
    let escrito = false;
    try {
      const s = await sesion();
      let html = await s.pedir(urlEdicionQuote(s.base, quoteNo));
      ...
      // La lista del encabezado viaja en ESTE mismo POST (#403): es el unico
      // ProcessOrder del ciclo, y FA no re-precia las partidas al recibirla (medido
      // sobre el quote 1287). Las opciones se leen del formulario que quedo despues
      // de re-armar el carrito, que es el que se esta posteando.
      const decisionLista = decidirListaQuote({ esperado: lista, actual: campos.sales_type, opciones: opcionesListaQuote(html) });
      // El domicilio de entrega viaja en el MISMO ProcessOrder (#409), por el mismo
      // motivo que la lista: es el unico de este ciclo. Sale de la cotizacion, que
      // desde este ticket guarda el branch_code que el vendedor eligio.
      const decisionBranch = decidirBranchQuote({
        esperado: data?.cliente?.branchId ?? null, actual: campos.branch_id, opciones: opcionesBranchQuote(html),
      });
      // El almacen del que se entrega ANTES de escribir, por nombre: FA lo deriva del
      // `default_location` del domicilio, asi que mover el domicilio puede moverlo
      // (#409, la 1288). Se toma del select del formulario que se esta posteando.
      // El transportista de la linea de envio elegida (#448), tercer gemelo del mismo
      // ProcessOrder: cambiar la linea de un quote ya subido lo mueve.
      const decisionTransportista = decidirTransportistaQuote({
        esperado: transportista, actual: campos.ship_via, opciones: opcionesTransportistaQuote(html),
      });
      const almacenAntes = nombreDeOpcion(opcionesDeSelect(html, 'Location'), campos.Location);
      const body = serializarBodyQuote(campos, {
        deliveryDate: vigencia, comments, custRef, deliverTo, deliveryAddress, contactPhone, contactEmail,
        ...(decisionLista.escribir ? { salesType: decisionLista.salesType } : {}),
        ...(decisionBranch.escribir ? { branchId: decisionBranch.branchId } : {}),
        ...(decisionTransportista.escribir ? { shipVia: decisionTransportista.shipVia } : {}),
      });
      escrito = true;
      ...
      // La lista no esta en la vista read-only, asi que se verifica por la API y se
      // suma al mismo veredicto: una lista que se escribio y NO quedo es una
      // divergencia del documento en Operam, igual que una partida que no pego. Lo
      // que NO se escribio (sin select, lista fuera de las opciones, sin lista
      // resoluble) sale con su motivo y no tumba la actualizacion: el contenido si
      // quedo, y el encabezado ya estaba asi antes.
      const pasoLista = decisionLista.escribir
        ? await verificarListaEscrita(quoteNo, decisionLista.salesType, leerLista)
        : listaSinEscribir(lista, decisionLista);
      const discrepanciaLista = pasoLista.escrita && pasoLista.verificado && !pasoLista.ok
        ? { campo: 'lista', esperado: pasoLista.esperado, encontrado: pasoLista.encontrado }
        : null;
      // El transportista NO entra al veredicto (#448): un transportista que no pego no
      // tumba la actualizacion -- partidas, vigencia y lista si quedaron -- y sale como
      // paso propio con lo que se leyo.
      const pasoTransportista = decisionTransportista.escribir
        ? await verificarListaEscrita(quoteNo, decisionTransportista.shipVia, leerTransportista, 'el transportista del encabezado')
        : listaSinEscribir(transportista, decisionTransportista);
      return {
        ...r,
        ok: r.ok && !discrepanciaLista,
        discrepancias: discrepanciaLista ? [...r.discrepancias, discrepanciaLista] : r.discrepancias,
        escrito: true,
        lista: pasoLista,
        transportista: pasoTransportista,
        // El almacen NO es un error ni tumba nada: es dato del ERP que el cotizador
        // no decide. Se reporta porque cambia SOLO, arrastrado por el domicilio, y
        // sin decirlo el pedido acaba saliendo de un almacen que nadie eligio.
        almacen: cambioDeAlmacen(almacenAntes, leerAlmacenVista(vista)),
        esperado: { items, comments, vigencia, lista: pasoLista.esperado },
      };
    } catch (err) {
      return {
        ok: false, escrito, verificado: false, discrepancias: [],
        lista: listaNoEscrita('la actualizacion del quote fallo antes de poder verificar la lista'),
        transportista: transportistaNoEscrito('la actualizacion del quote fallo antes de poder verificar el transportista'),
        error: err.message,
      };
    }
  });
...
// Id interno de Operam del segmento "Sin segmento", el que Operam le pone a todo cliente
// nuevo (el POST /customers ignora el que se le mande). Aqui es una REGLA, no una fila del
// catalogo: el catalogo completo vive en SEGMENTOS (server.js).
const SEGMENTO_SIN_ASIGNAR = '1';

// Escribe el segmento del cliente por la web legacy y devuelve
// { ok, yaCorrecto, conservado, actual, error }.
//
// `soloSinSegmento` (#186, decision de Adrian): con true, a un cliente que YA esta
// clasificado en Operam NO se le toca el segmento -- se devuelve { ok: true, conservado:
// true, actual }. Lo usan los dos caminos del ALTA, donde el segmento viaja de pasada
// junto a la cotizacion o al formulario: un cliente clasificado antes (en Operam o por
// otro vendedor) no puede perder su clasificacion porque en esta alta se eligiera otra
// cosa. Un cliente recien creado siempre cae aqui como "Sin segmento", asi que la misma
// regla cubre crear y reutilizar sin que el llamador tenga que distinguirlos. El upgrade
// fiscal NO la usa: ahi el vendedor edita la ficha del cliente a proposito y su seleccion
// manda (asi se verifico #172 en vivo, 1 -> 14 -> 1).
//
// Contrato: NUNCA lanza -- el llamador ya aplico lo suyo por la API y un fallo de este
// post-fix no debe tumbarlo; solo deja el segmento sin aplicar. No verifica releyendo: en
// el upgrade fiscal quien lo llama relee el cliente por la API v3 inmediatamente despues,
// y esa es la comprobacion real (200 de FA no garantiza nada, mismo quirk que el resto de
// la web legacy).
export async function actualizarSegmentoClienteWeb(clienteId, segmentoId, { soloSinSegmento = false } = {}) {
  return enCola(async () => {
    try {
      const s = await sesion();
      ...
      // La ficha recien leida es la fuente de "como esta hoy": preguntarselo a la API v3
      // seria un GET extra y ademas dejaria una ventana entre la lectura y la escritura.
      if (soloSinSegmento && String(campos.segmento_id) !== SEGMENTO_SIN_ASIGNAR) {
        return { ok: true, yaCorrecto: false, conservado: true, actual: String(campos.segmento_id) };
      }
