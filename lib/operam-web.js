...
// El maxlength REAL del textarea item_description de Operam. Un texto mas largo lo
// truncaria el ERP en silencio y el quote diria algo distinto del documento del
// cliente. Espejo de MAX_DESCRIPCION (public/js/descripcion-logica.js, el nucleo que
// frena la captura): lib/ no se sirve al navegador, por eso el numero vive dos veces.
const MAX_DESCRIPCION_OPERAM = 1000;
...
export function serializarBodyDescripcionLinea(campos, { descripcion } = {}) {
  ...
  if (texto.length > MAX_DESCRIPCION_OPERAM) {
    throw new Error(`La descripcion de la partida no cabe en Operam: ${texto.length} caracteres, el maximo es ${MAX_DESCRIPCION_OPERAM}`);
  }
