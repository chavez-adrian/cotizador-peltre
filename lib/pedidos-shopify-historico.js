...
// app: este modulo traduce las DOS fuentes posibles de la historia a la MISMA
// forma de nodo que consume `ingerirPedido` (lib/pedidos-shopify-logica.js).
// NO hay una segunda regla de telefono: todo pasa por ese nucleo.
//   1. CAMINO PRINCIPAL: la exportacion de PEDIDOS del admin de Shopify (trae
//      las dos direcciones completas por pedido, el cruce con Operam sobra).
//   2. Respaldo, si no hay export de pedidos: la exportacion de CLIENTES de
//      Shopify (telefono, pais, correo) cruzada con los pedidos `S` de Operam
//      (cliente de canal 184).
//
...
// telefono, no este modulo (ninguna lista cerrada de paises aqui). Cualquier
// otra cosa (vacio, o texto que no sean 2 letras) no se reconoce.
...
// El export del admin repite una fila POR RENGLON DE PRODUCTO del mismo
// pedido: la cabecera (Created at, direcciones, telefonos) solo viene llena en
// UNA de esas filas -- medido en vivo, es la primera, pero esta funcion no
// confia en el orden: se queda con la primera fila de cada `Name` que traiga
// `Created at`, y solo cae a la primera fila a secas si ninguna lo trae (Name
// sin cabecera reconocible en absoluto).
function agruparPorPedido(filasPedidos) {
  ...
  for (const filaPedido of filasPedidos || []) {
    ...
    if (!porNombre.has(nombre)) {
      porNombre.set(nombre, filaPedido);
      orden.push(nombre);
    } else if (!limpio(porNombre.get(nombre)['Created at']) && limpio(filaPedido['Created at'])) {
      porNombre.set(nombre, filaPedido);
    }
  }
  return orden.map(nombre => porNombre.get(nombre));
}

// Orquestacion PURA del modo pedidos-CSV (#258, CAMINO PRINCIPAL cuando el
// dueno entrega el export de PEDIDOS del admin: el cruce con Operam sobra).
// Los pedidos cancelados (`Cancelled at` con fecha) ENTRAN igual (decision del
// dueno: todos los pedidos respaldan una ficha, ADR-0014 "para siempre") --
// este modulo no mira esa columna.
export function planearCargaDesdePedidosCsv({ filasPedidos } = {}) {
  ...
  let conCodigo = 0;
  let porPais = 0;
  const pedidos = agruparPorPedido(filasPedidos);
  for (const filaPedido of pedidos) {
  ...
  return { leidos: pedidos.length, filas, descartes, conCodigo, porPais, telefonosDistintos: telefonos.size };
}
