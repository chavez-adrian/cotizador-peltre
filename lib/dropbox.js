...
// se repite en ningun sitio de llamada. El registro nunca lanza (el store se
...
export async function upload(destino, content, mode = 'overwrite') {
  ...
  try {
    ...
    await registrarSubida({ flujo: config.flujo, destino: path, ok: true });
    return data;
  } catch (err) {
    await registrarSubida({ flujo: config.flujo, destino: path, ok: false, error: err.message });
    throw err;
  }
}
