...
export async function generateQuotePDF(data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // cliente y el ERP salen de la misma fuente. Aqui NO se trunca -- el limite de
    // 60 es del quote --, asi que un escalon de fallback mas largo que eso sale
    // completo en el documento y cortado en Operam.
    ...
    // interno del registro: quien recibe el documento y quien abre el ERP tienen
    // que leer el mismo numero. Lo resuelve un solo punto del server y llega aqui
    // ya decidido; sin folio el documento simplemente no lleva numero.
    ...
    // Sin folio el documento no lleva numero, asi que el encabezado es lo unico
    // que lo distingue de una cotizacion registrada (ADR-0009). "PRE-COTIZACION"
    // no cabe a 22pt en la columna derecha: va a 16.
    doc.fontSize(folio ? 22 : 16).font('Helvetica-Bold').fillColor('#000000')
      .text(folio ? 'COTIZACION' : 'PRE-COTIZACION', derechaX, y + 8, { width: colW3, align: 'right' });
    ...
    if (folio) {
      doc.fillColor('#C05000').font('Helvetica-Bold')
        .text(`No. Cotizacion: ${folio}`, derechaX, metaY, { width: colW3, align: 'right' });
    ...
    const telCorreo = (c.celEntrega || c.emailEntrega)
      ? `Telefono: ${c.celEntrega || ''}${c.emailEntrega ? ` , Correo: ${c.emailEntrega}` : ''}`
      : '';
    ...
    const rightLines = [
      ...
      c.leyendaDomicilio || '',
      telCorreo,
      refCliente ? `Referencia Cliente: ${refCliente}` : '',
    ].filter(Boolean);
    ...
    // Paginacion
    // height explicito: evita que PDFKit dispare un salto de pagina automatico
    // al escribir por debajo del margen inferior (doc.page.maxY() = height - margin.bottom)
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc.fontSize(7).fillColor('#999999').font('Helvetica')
        .text(`Pagina ${i + 1} de ${pageCount}`,
          ...
          { width: CONTENT_W, height: 20, align: 'right' });
    }
