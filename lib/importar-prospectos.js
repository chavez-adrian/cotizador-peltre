...
// senales de calificacion del cuestionario del evento: no tienen campo propio
// en el prospecto, asi que van como UNA linea de notas debajo de la nota del
// vendedor.
function lineaCalificacion(campos) {
  return [
    ...
    ['Área de interés', campos.areaInteres],
...
export function importarProspectosFeria(buffer, { vendedores = [], vendedorDefault, evento } = {}) {
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    const notas = [campo('nota'), lineaCalificacion({
      ...
      tamano: campo('tamano'), decision: campo('decision'), areaInteres: campo('areaInteres'),
    })].filter(Boolean).join('\n');
