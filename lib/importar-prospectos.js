...
import { segmentoDeTipo, capitalizarCampo, normalizarTextosProspecto } from '../public/js/prospectos-logica.js';
...
// La regla de capitalizacion es la de la casa (capitalizarCampo, issue #235 y
// #269): "venga de donde venga la captura", lo que incluye este importador.
// Antes vivia aqui una funcion `titular` propia y mas pobre -- sin particulas
// ("Los Antojos DEL Gordo") ni siglas fijas ("Sa De Cv") -- y ademas solo se
// aplicaba al nombre de la persona, asi que la empresa y la ciudad se guardaban
// como venian del gafete.
...
export function importarProspectosFeria(buffer, { vendedores = [], vendedorDefault, evento } = {}) {
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    const nombre = capitalizarCampo([campo('nombre'), campo('apellido')].filter(Boolean).join(' '));
    const empresa = capitalizarCampo(campo('empresa'));
    // El correo lo baja a minusculas normalizarTextosProspecto al armar la fila;
    // aqui se lee crudo porque el gafete sin celular lo reporta aparte.
    ...
    if (!celularCrudo) {
      ...
      const normalizado = normalizarTextosProspecto({ nombre, data });
      sinCelular.push({
        fila, nombre, empresa, correo: normalizado.data.correo || correo,
        scoring: temperatura === undefined ? '' : temperatura, data: normalizado.data,
      });
      continue;
    }
    ...
    // (normalizarTextosProspecto): ciudad capitalizada y correo en minusculas,
    // sin una segunda copia de la regla aqui. Conserva las llaves que no le
    // tocan (fila, celular, canal, vendedor).
    listos.push(normalizarTextosProspecto({
      fila,
      celular,
      nombre,
      correo,
