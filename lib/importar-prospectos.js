...
import { TIPOS_CLIENTE, segmentoDeTipo } from '../public/js/prospectos-logica.js';
...
// los gafetes sin celular y las descartadas con motivo; el cruce contra
// prospectos existentes y clientes Operam vive en la ruta, porque ahi es donde
// enriquecer o crear es una decision con IO.
...
const COLUMNAS = {
  ...
  decision: 'Decisión de compra (es)',
  scoring: 'Scoring',
  nota: 'Note',
...
// Los nombres del export llegan en MAYUSCULAS y asi se verian en la libreta de
// WhatsApp. Se titula palabra por palabra y SOLO la que no trae minusculas, para
// no romper lo que ya venia bien escrito ("McKenzie").
function titular(texto) {
  return limpiar(texto).split(/\s+/).filter(Boolean).map(palabra =>
    palabra !== palabra.toUpperCase() ? palabra : palabra.charAt(0).toUpperCase() + palabra.slice(1).toLowerCase()
  ).join(' ');
}
...
function tipoClienteDeActividad(actividad) {
  ...
  const tipo = ACTIVIDAD_A_TIPO[llaveTexto(texto)];
...
// Puesto, tamano de empresa y decision de compra son las senales de calificacion
// del cuestionario del evento: no tienen campo propio en el prospecto, asi que
// van como UNA linea de notas debajo de la nota del vendedor.
function lineaCalificacion(campos) {
  return [
    ['Puesto', campos.puesto || campos.jobTitle],
    ['Tamaño de empresa', campos.tamano],
    ['Decisión de compra', campos.decision],
...
function indicesDeCabeceras(headers) {
  ...
  for (const [campo, cabecera] of Object.entries(COLUMNAS)) {
    const i = porLlave.get(llaveTexto(cabecera));
...
export function importarProspectosFeria(buffer, { vendedores = [], vendedorDefault, evento } = {}) {
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    const nombre = titular([campo('nombre'), campo('apellido')].filter(Boolean).join(' '));
    ...
    if (tipo) Object.assign(data, tipo);
    const temperatura = temperaturaDeScoring(campo('scoring'));
    if (temperatura !== undefined) data.temperatura = temperatura;
    const notas = [campo('nota'), lineaCalificacion({
      puesto: campo('puesto'), jobTitle: campo('jobTitle'),
      tamano: campo('tamano'), decision: campo('decision'),
    })].filter(Boolean).join('\n');
    ...
    if (!celularCrudo) {
      ...
      sinCelular.push({ fila, nombre, empresa, correo, scoring: temperatura === undefined ? '' : temperatura, data });
      continue;
    }
