...
// fila que guardo su namespace lo dice sola. La que no -- toda la anterior a
// #399 -- se INFIERE por su ruta y se marca `inferido`: sin namespace la
// peticion sale con la ruta heredada del flujo, asi que lo que cuelga de ella
// es sandbox, y lo que no es una ruta RELATIVA a un namespace (las filas que
// viajaron con namespace antes de que el registro supiera guardarlo). El flujo
...
export function lugarDeSubida(fila, env = process.env) {
  ...
  return enSandbox
    ? { tipo: 'sandbox', namespace: null, inferido: false }
    : { tipo: 'namespace', namespace: null, inferido: true };
}
