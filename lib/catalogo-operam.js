...
// Las listas SIN escalon de volumen (#298/#299, ADR-0015): el cotizador las precia con
// la MISMA regla que a los escalones -- fila explicita gana, si no base x factor --
// pero NO tienen min_qty. Jamas participan en el tabulador (Auto sigue decidiendo solo
// entre los escalones de volumen en pesos) y solo se alcanzan fijandolas.
//
// NO son una tabla en codigo: son las sales_types ACTIVAS de Operam que no son escalon
// de volumen -- hoy Segundas (condicion del producto), Amazon (canal, la unica mas cara
// que el precio base) y M6001 mas las cinco US100-US6000 --, porque el ERP es la fuente
// de verdad de que listas existen (ADR-0015; CONTEXT.md "Lista de precios": el universo
// es el de Operam, no un catalogo fijo del cotizador -- una copia en codigo se
// desactualiza, como el selector del alta que vivio meses mostrando 10 de 14). Una
// lista nueva en el ERP queda cotizable con solo regenerar el catalogo.
//
// Los dos filtros fallan CERRADO: la lista INACTIVA no entra aunque tenga filas de
// precio vivas (Bazaar tiene 461), y la que no trae factor legible tampoco -- sin
// factor no hay con que preciarla, y dejarla entrar sin precios la haria cobrar
// menudeo por el `prices['Menudeo'] ?? 0` de app.js. El orden es por id de Operam:
// el catalogo generado no puede depender del orden en que el ERP devuelva las paginas.
function listasSinEscalon(salesTypes, listasVolumen) {
  const ocupadas = new Set(Object.values(listasVolumen).map(l => String(l.listaId)));
  const nombresDeTier = new Set(TIERS.map(t => t.id));
  return (salesTypes || [])
    .filter(s => s.inactive !== '1' && s.inactive !== 1)
    .filter(s => Number(s.factor) > 0)
    .filter(s => s.sales_type && !ocupadas.has(String(s.id)) && !nombresDeTier.has(s.sales_type))
    .sort((a, b) => Number(a.id) - Number(b.id))
    .map(s => ({ id: s.sales_type, label: s.sales_type, lista: s.sales_type, idRespaldo: String(s.id) }));
}

function buscarSalesType(salesTypes, lista) {
  return (salesTypes || []).find(s => s.sales_type === lista.lista)
    || (salesTypes || []).find(s => String(s.id) === lista.idRespaldo);
}

function resolverListas(salesTypes, listas) {
  const resueltas = {};
  for (const lista of listas) {
    const st = buscarSalesType(salesTypes, lista);
    resueltas[lista.id] = st
      ? { listaId: String(st.id), factor: Number(st.factor) }
      : { listaId: lista.idRespaldo, factor: null };
  }
  return resueltas;
}
...
function preciosDeSku(filas, listas, todasLasListas) {
  ...
  const prices = {};
  for (const tier of todasLasListas) {
...
// nadie cobra. La agrupacion es por las listas COMPLETAS, no solo por la base: dos
...
// Desde #298 eso incluye a las listas sin escalon, por el mismo motivo (M6001 trae
// filas propias). Empate en numero de SKUs -> gana el mas caro en la primera lista
// donde difieran (nunca se cobra de menos por accidente).
function precioRepresentante(conBase, todasLasListas) {
  ...
  for (const p of conBase) {
    ...
    const llave = todasLasListas.map(t => (p.prices[t.id] == null ? '-' : p.prices[t.id].toFixed(2))).join('|');
...
// es NUEVO. Se comparan los ESCALONES DE VOLUMEN (#298): la referencia del corte #131
// salio del Excel, que nunca tuvo las listas sin escalon -- compararlas marcaria cada
// producto como MISMATCH por un precio que la referencia no conoce.
...
export function construirCatalogo({ salesTypes, precios, items, complemento, referencia = null, version = null, extracted = null } = {}) {
  ...
  // Las listas sin escalon salen de las sales_types ACTIVAS que el ERP reporte
  // (ADR-0015): desactivar una en Operam la retira del cotizador con efecto inmediato y
  // deja de ser fijable por nadie. Los escalones de volumen no llevan ese filtro: son
  // el tabulador y tienen id de respaldo, no pueden quedarse fuera del catalogo, y por
  // eso se resuelven PRIMERO -- su lista ya resuelta es la que queda excluida de las
  // sin escalon, aunque alguien la haya renombrado en el ERP.
  const listasVolumen = resolverListas(salesTypes, TIERS);
  const sinEscalon = listasSinEscalon(salesTypes, listasVolumen);
  const todasLasListas = [...TIERS, ...sinEscalon];
  const listas = { ...listasVolumen, ...resolverListas(salesTypes, sinEscalon) };
  ...
  for (const item of activos) {
    const suPrecio = preciosDeSku(filasPorSku.get(item.stock_id), listas, todasLasListas);
  ...
  for (const [clave, grupo] of porClave) {
    ...
    const representante = precioRepresentante(conBase, todasLasListas);
  ...
  const catalogo = {
    ...
    // `listaId` (#296, ADR-0015): el id de la sales_type de Operam que precia
    // ese escalon, ya resuelto por nombre contra el ERP. Es por donde el
    // permiso de lista (matriz vendedor x lista) cruza con el tier: sin el, un
    // tier no es fijable por nadie mas que el admin.
    // Las listas sin escalon van al FINAL y SIN la llave min_qty (#298): asi el
    // tabulador no las puede tomar ni siquiera por el primer elemento de la lista.
    tiers: [
      ...TIERS.map(({ id, label, min_qty }) => ({ id, label, min_qty, listaId: listas[id].listaId })),
      ...sinEscalon.map(({ id, label }) => ({ id, label, listaId: listas[id].listaId })),
    ],
    boxMap: comp.boxMap || [],
    skus,
