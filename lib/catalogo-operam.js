...
export function construirCatalogo({ salesTypes, precios, items, complemento, referencia = null, version = null, extracted = null } = {}) {
  ...
  // La comparacion de calcas es por codigo BASE: la referencia del Excel dice CAL1025
  // y el catalogo puede traer CAL1025S (misma calca, unidad migrada) -- compararlas
  // por stock_id literal inventaria un SIN_SKU y un NUEVO falsos por cada migracion.
  const calcasComparables = calcas.map(c => ({ ...c, code: c.code.replace(/S$/, '') }));
  const filasCalcas = clasificar(calcasComparables, referencia?.calcas, 'code', TIERS.filter(t => t.id !== 'Menudeo'));
