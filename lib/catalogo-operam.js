...
// Las listas SIN escalon de volumen (#298, ADR-0015): el cotizador las precia con la
// MISMA regla que a los escalones -- fila explicita gana, si no base x factor -- pero
// NO tienen min_qty. Jamas participan en el tabulador (Auto sigue decidiendo solo
// entre los escalones de volumen en pesos) y solo se alcanzan fijandolas. Segundas
// (producto con defecto, factor 0.165) estrena el mecanismo; las otras siete listas
// activas de Operam entran por #299. Misma tabla que TIERS menos el min_qty, y la
// resolucion es igual: por NOMBRE, con el id de respaldo si alguien la renombra.
const LISTAS_SIN_ESCALON = [
  { id: 'Segundas', label: 'Segundas', lista: 'Segundas', idRespaldo: '9' },
];
...
export function construirCatalogo({ salesTypes, precios, items, complemento, referencia = null, version = null, extracted = null } = {}) {
  ...
  // Una lista sin escalon entra al catalogo SOLO si Operam la reporta activa
  // (ADR-0015): desactivarla en el ERP la retira del cotizador con efecto inmediato y
  // deja de ser fijable por nadie. Los escalones de volumen no llevan ese filtro: son
  // el tabulador y tienen id de respaldo, no pueden quedarse fuera del catalogo.
  const sinEscalon = LISTAS_SIN_ESCALON.filter(l => {
    const st = buscarSalesType(salesTypes, l);
    return !!st && st.inactive !== '1' && st.inactive !== 1;
  });
  const todasLasListas = [...TIERS, ...sinEscalon];
  const listas = resolverListas(salesTypes, todasLasListas);
