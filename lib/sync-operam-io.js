...
import { etapaPostVenta, hechosDesdeOperam, estadoPago, pagoSinRegistrar } from './sync-operam.js';
...
export async function hechosDeOperam(op, deps = {}) {
  ...
  const rfc = rfcDeOportunidad(op);
  if (customerId == null && !rfc) return null; // sin binding key no se puede ligar
...
export async function reconciliarOportunidad(op, deps = {}) {
  ...
  if (op && op.id != null) {
    await _actualizarDatos(op.id, { pagoSinRegistrar: pagoSinRegistrar(hechos) });
  }
...
// ausente) si es terminal.
const TERMINALES = new Set(['no_util', 'perdida']);
export function esActivaPostVentaCandidata(op) {
  if (op == null) return false;
  if (TERMINALES.has(op.etapa)) return false;
