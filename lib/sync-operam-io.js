...
// el reflejo de folios de la cadena post-venta para trazabilidad en la tarjeta:
//   { cotizacion, pedido, factura:{numero,ref}, remisiones[], pagos[], notasCredito[] }
// Tipos (peltre-operam.md 12): 10=factura, 13=remision, 12=pago, 11=nota de credito.
// Solo incluye lo que EXISTE: si no hay factura aun, no pone la clave; si no hay
// remisiones/pagos/notas, deja el arreglo vacio (forma estable para la tarjeta).
export function construirEspejoOperam(transFiltradas, pedidosFiltrados, folio) {
  ...
  const espejo = { remisiones: [], pagos: [], notasCredito: [] };
  ...
  for (const t of lista) {
    if (!t) continue;
    const ty = tipo(t);
    if (ty === 10 && !espejo.factura) {
      espejo.factura = { numero: String(t.trans_no ?? ''), ref: String(t.ref ?? '') };
    } else if (ty === 13) {
      espejo.remisiones.push(String(t.trans_no ?? t.ref ?? ''));
    } else if (ty === 12) {
      espejo.pagos.push(String(t.ref ?? t.trans_no ?? ''));
    } else if (ty === 11) {
      espejo.notasCredito.push(String(t.ref ?? t.trans_no ?? ''));
    }
  }
  return espejo;
}
...
// filtra a ese order_; si no, se usa el agregado por cliente. Cuando hay binding
// preciso (order_ resuelto por documento/explicito), adjunta el espejo de la cadena
// (_order, _espejo) para que reconciliarOportunidad lo persista (#67, AC3).
export async function hechosDeOperam(op, deps = {}) {
  ...
  // Espejo de la cadena (#67, AC3): SOLO con binding preciso (order_ resuelto por
  // documento o explicito). El agregado por cliente no es una liga precisa, asi que
  // no se persiste un espejo que mezclaria cadenas. Una venta directa nunca resuelve
  // order_ por documento -> no liga -> no espejo.
  if (ligaAlOrder) {
    hechos._order = order;
...
export async function reconciliarOportunidad(op, deps = {}) {
  ...
  // Persiste el espejo de la cadena (#67, AC3) cuando hubo binding preciso, aunque
  // la etapa no cambie: la trazabilidad de folios vale por si misma. Solo aplica a
  // cotizaciones (las que tienen folioOperam); el motor lo expone via hechos._espejo.
  if (hechos._espejo && op && op.id != null) {
