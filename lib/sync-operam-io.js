...
// tarjeta. El webhook es solo una SEÑAL ("algo cambio para este cliente/order_");
...
//
// Diseño (peltre-operam.md seccion 12, sesion HITL #62):
//   - La cadena post-venta se une por `order_`/`order_no`. Una oportunidad =
//     un pedido (CONTEXT.md "Oportunidad"). El folio de #63 (data.folioOperam,
//     quote_id de POST /api/v3/sales/quote) es el handle de entrada.
...
//   - Si la oportunidad resuelve a un order_ concreto, la cadena se filtra a ese
//     order_ (binding preciso). Si no (no se puede ligar el quote_id a un order_
//     de forma fiable), se cae al agregado por cliente (correcto en el caso comun:
...
// El order_ al que esta ligada la oportunidad, si se puede resolver. data.orderOperam
// es la liga explicita (la pone el caller cuando la conoce). Si no, el folio de #63
// se usa como candidato (puede o no coincidir con un order_no real -- ver NOTA).
function orderDeOportunidad(op) {
  const explicito = op?.data?.orderOperam;
  if (explicito != null && explicito !== '') return String(explicito);
...
export async function reconciliarPorIdentificador(identificador, oportunidades, deps = {}) {
  ...
  if (order) {
    const exactas = candidatas.filter(o =>
      String(o?.data?.orderOperam ?? o?.folioOperam ?? '') === order
    );
...
// NOTA (valvula de seguridad, decision de dominio pendiente): no se confirmo en
// vivo si el quote_id que devuelve POST /api/v3/sales/quote (#63) ES el order_no
// del Sales Order, o si la confirmacion del quote a pedido genera un order_no
// distinto. Por eso el binding por order_ es BEST-EFFORT: si el folio coincide con
// un order_ real, se filtra a esa cadena; si no, se cae al agregado por cliente
// (correcto cuando el cliente tiene una sola oportunidad activa -- el caso comun
// de Peltre). Cuando se capture la liga real (o el webhook traiga el order_), basta
// poblar data.orderOperam en la oportunidad para binding preciso, sin tocar el
// motor. Si un cliente tiene varias oportunidades activas a la vez SIN order_
// resuelto, la reconciliacion podria mover la etapa equivocada: ese caso necesita
// la liga explicita (data.orderOperam) y se documenta en PROGRESS.
