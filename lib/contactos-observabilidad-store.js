...
export async function guardar(nombreBarrido, estado) {
  await ensureSchema();
  const {
    ultimaCorrida, ultimaCorridaExitosa, creados, actualizados, inactivados, errores, ultimoAviso,
  } = estado;
  const r = await query(
    `INSERT INTO contactos_google_barridos
       (nombre_barrido, ultima_corrida, ultima_corrida_exitosa, creados, actualizados, inactivados, errores, ultimo_aviso)
