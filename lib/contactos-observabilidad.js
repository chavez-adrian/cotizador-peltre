...
//
// Un "barrido" se identifica por su nombre (hoy uno solo, 'contactos', que
// desde #228 cubre prospectos Y clientes de Operam en la misma pasada): el
// estado se guarda por nombre para que un barrido nuevo entre sin tocar este modulo.
...
//
// `totales` (issue #257) es un objeto LIBRE que cada barrido llena a su manera
// (el de contactos no lo usa; el sondeo de pedidos de Shopify guarda ahi
// leidos/filas/descartesPorMotivo, forma que este modulo NO conoce): se
// conserva tal cual, sin que un barrido nuevo tenga que tocar este modulo.
//
...
export function siguienteEstado(estadoPrevio, resumen, ahora = new Date()) {
  ...
  // Un error que YA viene clasificado conserva su categoria: el plan de
  // contactos (#231) sabe que su tope de inactivacion es un problema de datos, y
  // su motivo no trae codigo de Google que la tabla de arriba pueda reconocer.
  const errores = (r.errores || []).map(e => ({
    celular10: e.celular10,
    motivo: e.motivo,
    categoria: e.categoria || clasificarError(e.motivo),
  }));
...
const ETIQUETAS_BARRIDO = {
  contactos: 'prospectos y clientes de Operam',
