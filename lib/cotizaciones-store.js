...
// Celular del Contacto de la Oportunidad (#342, ADR-0016): columna de primer
// nivel y NO una clave dentro de data, porque la liga es fija y data se
// reemplaza entero en cada regeneracion -- el mismo criterio que folio_operam
// (#63). Nullable: una cotizacion historica sin migrar queda "sin Contacto".
const SCHEMA_CONTACTO = `ALTER TABLE cotizaciones ADD COLUMN IF NOT EXISTS contacto_celular TEXT`;
...
function filaAEntrada(row) {
  return migrarCotizacion({
    ...
    contactoCelular: row.contacto_celular ?? null,
...
function entradaJson(c) {
  return migrarCotizacion({ ...c, folioOperam: c.folioOperam ?? null, contactoCelular: c.contactoCelular ?? null });
}
...
export async function crear(entry) {
  await ensureSchema();
  const r = await query(
    `INSERT INTO cotizaciones (id, fecha, vendedor, cliente, total_piezas, total, tier, data, contacto_celular)
     VALUES ((SELECT COALESCE(MAX(id), 0) + 1 FROM cotizaciones), $1, $2, $3, $4, $5, $6, $7, $8)
     RETURNING id`,
    [entry.fecha, entry.vendedor, entry.cliente, entry.totalPiezas || 0, entry.total || 0, entry.tier || '', JSON.stringify(entry.data || {}), entry.contactoCelular || null]
  );
  if (r === null) {
    const log = leerJson();
...
// Regeneracion de una cotizacion existente (issue #83, F1: 1 oportunidad = 1
// cotizacion): actualiza los campos de resumen y MERGEA el data a nivel raiz
// (||), para que las claves que la regeneracion no trae (decorado,
// calcaChecklist, espejoOperam...) sobrevivan. data.cliente si se reemplaza
// completo: preservar customerId/branchId ligados es responsabilidad del caller
// (la ruta), que los copia al data nuevo antes de llamar. No toca fecha,
// vendedor, estado, folio ni etapa.
export async function actualizarCotizacion(id, entry) {
  await ensureSchema();
  const r = await query(
    `UPDATE cotizaciones SET cliente = $2, total_piezas = $3, total = $4, tier = $5,
       data = COALESCE(data, '{}'::jsonb) || $6::jsonb
     WHERE id = $1`,
    [id, entry.cliente, entry.totalPiezas || 0, entry.total || 0, entry.tier || '', JSON.stringify(entry.data || {})]
  );
  if (r === null) {
    const log = leerJson();
    const e = log.find(c => c.id === id);
    if (!e) return false;
    e.cliente = entry.cliente;
    e.totalPiezas = entry.totalPiezas || 0;
    e.total = entry.total || 0;
    e.tier = entry.tier || '';
    e.data = { ...(e.data || {}), ...(entry.data || {}) };
    escribirJson(log);
    return true;
  }
  return r.rowCount > 0;
...
// Persiste el espejo de la cadena Operam de la oportunidad (issue #67, AC3): una
// vez resuelto el order_ por documento de origen, se guarda en data.espejoOperam la
// cadena de folios atribuibles (cotizacion, pedido, factura num+ref, remisiones) +
// el estado de pago derivado de la factura, para trazabilidad en la tarjeta sin
// entrar al ERP. Mergea en el data JSONB (mismo patron que actualizarDatos),
// reemplazando solo la clave espejoOperam.
export async function setEspejoOperam(id, espejo) {
  await ensureSchema();
  const merge = JSON.stringify({ espejoOperam: espejo || {} });
  const r = await query(
    `UPDATE cotizaciones SET data = COALESCE(data, '{}'::jsonb) || $2::jsonb WHERE id = $1`,
    [id, merge]
  );
  if (r === null) {
    ...
    const entry = log.find(c => c.id === id);
    if (!entry) return false;
    entry.data = { ...(entry.data || {}), espejoOperam: espejo || {} };
    escribirJson(log);
    return true;
  }
  return r.rowCount > 0;
...
// Liga la Oportunidad con su Contacto (#342, ADR-0016). La liga es FIJA: solo
// escribe si la cotizacion no tiene ya un celular anotado, y devuelve false si
// lo tiene (ni la migracion ni la captura manual pisan una liga existente). Esa
// guarda vive AQUI y no en el caller para que el script y la ruta no puedan
// discrepar sobre que significa "fija" -- y es lo que hace idempotente a la
// migracion: la segunda corrida no escribe nada.
export async function setContactoCelular(id, celular) {
  await ensureSchema();
  const valor = celular == null ? null : String(celular);
  if (!valor) return false;
  const r = await query(
    `UPDATE cotizaciones SET contacto_celular = $2
     WHERE id = $1 AND (contacto_celular IS NULL OR contacto_celular = '')`,
    [id, valor]
  );
  if (r === null) {
    const log = leerJson();
    const entry = log.find(c => c.id === id);
    if (!entry) return false;
    if (entry.contactoCelular) return false;
    entry.contactoCelular = valor;
    escribirJson(log);
    return true;
  }
  return r.rowCount > 0;
...
// Borra una cotizacion por id (issue #75, limpieza de datos de prueba). Devuelve
// true si elimino una fila, false si no existia (idempotente). Scoped a la tabla
// cotizaciones; lo usa scripts/limpiar-datos-prueba.mjs tras respaldar.
export async function borrar(id) {
  await ensureSchema();
  const r = await query('DELETE FROM cotizaciones WHERE id = $1', [id]);
  if (r === null) {
    const log = leerJson();
