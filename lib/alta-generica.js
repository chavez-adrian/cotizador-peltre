...
// RFC generico de ESTE cliente (#244). El RFC capturado es un HECHO; el pais es
// una inferencia de respaldo, y una mala: al elegir un cliente de Operam el
// formulario deja el pais en MX siempre (pcLimpiarCamposCliente lo resetea,
// seleccionarClienteOperam no lo toca y GET /api/operam/clientes ni siquiera lo
// devuelve). Derivar del pais hacia que un cliente con XEXX ya capturado se diera
// de alta con tax_id XAXX, contradiciendo el documento que el cliente ya vio, y
// que la dedup preguntara por el pool equivocado. Cuando el RFC generico ya esta
// capturado, ese RFC manda. Un RFC real no llega aqui (necesitaAltaGenerica lo
// frena) y, si llegara, se ignora: no puede volverse el tax_id de un generico.
export function rfcGenericoDe(cliente) {
  const c = cliente || {};
  const rfc = normalizarRfc(c.rfc);
  return RFC_GENERICOS.has(rfc) ? rfc : rfcGenericoPara(c.pais);
}
...
// lo que /api/crear-cliente ya lleva al branch; br_ref no sale de aqui, pero el
// PUT lo EXIGE (#386): lo rellena darDeAlta con la referencia que relee del branch
// que Operam auto-creo en el POST. addr_reference viene de
...
export function buildBranchGenerico(cliente, { salesman } = {}) {
  ...
  // Cel de la sucursal (#339): el celular del CONTACTO, no el de la entrega, y
  // fuera del gate del domicilio -- es un dato de la persona, no de la direccion,
  // asi que una sucursal sin calle ni CP tambien lo lleva.
  if (c.telefono) datos.fax = c.telefono;
  if (calle && cp) {
    datos.addr_street = calle;
    // Numero exterior y referencia corta: el paso Envio no los captura (la calle
    // carga calle+numero y la referencia la auto-crea Operam), el alta completa
    // SI (#366). Vacios no cambian nada; con valor viajan al mismo body.
    datos.addr_exterior = c.numExt || '';
  ...
  if (c.refEntrega) datos.br_ref = c.refEntrega;
...
// El tercer elemento es el LECTOR (#431): `phone` y `email` no vienen en el
// GET /branches/:code (ese GET no expone las llaves de contacto) y se leen de la
// entrada del domicilio en `branches[]` de GET /customers/:id.
const CAMPOS_BRANCH = [
  ['br_name', 'Nombre'],
  ['addr_street', 'Calle'], ['addr_exterior', 'Numero exterior'], ['addr_interior', 'Numero interior'], ['addr_colony', 'Colonia'],
  ['addr_city', 'Municipio'], ['addr_state', 'Estado'], ['addr_zip', 'CP'],
  ['addr_reference', 'Referencias'], ['phone', 'Telefono', 'cliente'], ['email', 'Email', 'cliente'],
];

// Los campos que solo se leen del domicilio en `branches[]` del cliente: quien
// verifica los mira para saber si tiene que releer al cliente.
export const CAMPOS_BRANCH_DEL_CLIENTE = CAMPOS_BRANCH.filter(([, , lector]) => lector === 'cliente').map(([campo]) => campo);
...
// intentaron escribir (no vacios) y no coinciden. El telefono se compara por
// digitos (#369): el formulario lo manda como "+52 2222933000" y Operam lo
// persiste sin el espacio; como texto salia un falso "ignoro phone".
// `branchDelCliente` es la entrada del mismo domicilio en `branches[]` de
// GET /customers/:id (#431). Sin ella esos campos NO se comparan: no se pudieron
// leer, y afirmar que Operam los ignoro seria inventar una diferencia (#373).
const soloDigitos = (v) => String(v ?? '').replace(/\D/g, '');

export function diffBranchDomicilio(branchFresco, enviado, branchDelCliente = null) {
  const e = enviado || {};
  const out = [];
  for (const [campo, label, lector] of CAMPOS_BRANCH) {
    const nuevo = String(e[campo] ?? '').trim();
    if (!nuevo) continue;
    const b = lector === 'cliente' ? branchDelCliente : (branchFresco || {});
    if (!b) continue;
    const coincide = campo === 'phone'
      ? soloDigitos(b[campo]) === soloDigitos(nuevo)
      : String(b[campo] ?? '').trim() === nuevo;
    if (!coincide) {
