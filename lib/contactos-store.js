...
// Olvida la fila (#249): a diferencia de marcarInactivo, esta SI borra -- es
// para el caso en que la ficha de Google ya no existe (404 en un PATCH o en la
// relectura por etag), no para el que perdio respaldo pero sigue viva. Sin
// fila, la siguiente pasada trata el celular como nuevo: lo CREA de vuelta o lo
// ADOPTA si hay una ficha manual con ese numero en la libreta. No hay DELETE a
// People API por ningun camino (ADR-0013); esto borra solo la fila del mapeo.
export async function eliminar(celular10) {
  await ensureSchema();
  const r = await query('DELETE FROM contactos_google WHERE celular10 = $1', [celular10]);
  if (r === null) {
    const log = leerJson();
    const i = log.findIndex(e => e.celular10 === celular10);
    if (i === -1) return false;
