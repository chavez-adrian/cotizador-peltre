...
import { ORDEN_CASILLA } from './contacto-cotizacion.js';
...
// trae inline contacts[].phone/phone2/fax y branches[].phone/fax (verificado
// contra produccion 2026-06-10: 440 clientes, 549 telefonos en contacts y 263
// en branches, customer.phone siempre ausente). `fax` es la casilla "Cel" que
// la web muestra y la API renombra (#338/ADR-0016, CONTEXT.md "Contacto en
// Operam"): 68 celulares de 65 clientes solo viven ahi (medido 2026-09-05).
// Formatos inconsistentes, con extensiones (",116", "ext.123") que se recortan
// antes de tomar los ultimos 10 digitos; numeros de menos de 10 digitos no
// producen llave (limite documentado del best effort). Cache en memoria con
// TTL de 1 h y refresh bajo demanda; un fallo de Operam nunca lanza desde
// matchCliente.
...
export function enumerarTelefonosClientes(clientes) {
  ...
  for (const c of clientes || []) {
    ...
    // rol que atribuirle.
    agregar(c.phone, { fuente: 'contacto', casilla: 'telefono' });
    for (const ct of c.contacts || []) {
      ...
      // phone, phone2 y fax (Cel, #338/ADR-0016) son numeros de LA MISMA
      // persona: el Cel se enumera con su mismo persona/rol, nunca aparte. La
      // `casilla` (#342) es la de la PANTALLA de Operam, no la llave de la API:
      // el respaldo automatico de la migracion elige por ella (Cel > Telefono >
      // Telefono Secundario).
      agregar(ct.phone, { ...contacto, casilla: 'telefono' });
      agregar(ct.phone2, { ...contacto, casilla: 'telefono_secundario' });
      agregar(ct.fax, { ...contacto, casilla: 'cel' });
    }
    for (const b of c.branches || []) {
      ...
      // Igual que en el Contacto en Operam: el Cel (fax) de la sucursal es del
      // mismo contacto que su Telefono. La sucursal no tiene Telefono Secundario.
      agregar(b.phone, { ...domicilio, casilla: 'telefono' });
      agregar(b.fax, { ...domicilio, casilla: 'cel' });
    }
  }
  return entradas;
}

// Los numeros de UN Cliente Operam tal como los lee su fila (#426): los de sus
// seis casillas, en el orden de ADR-0016 (Cel > Telefono > Secundario,
// ORDEN_CASILLA) y, dentro de una casilla, en el de la enumeracion. El texto
// viaja como viene de Operam, con su codigo de pais (el widget elige la bandera
// con el); el mismo numero en dos casillas sale una vez, en la de mayor
// precedencia, comparado por sus ultimos 10 digitos (un texto sin ningun
// digito se compara tal cual). Los de menos de 10 digitos se quedan: la fila
// los mostraba antes de #426 y el vendedor decide que hacer con ellos.
export function telefonosDeClienteOperam(c) {
  const vistos = new Set();
  return enumerarTelefonosClientes([c])
    .sort((a, b) => ORDEN_CASILLA.indexOf(a.casilla) - ORDEN_CASILLA.indexOf(b.casilla))
    .map(e => e.telefono)
    .filter(t => {
      const llave = ultimos10(t) || t;
