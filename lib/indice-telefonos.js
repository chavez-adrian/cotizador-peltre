...
export function enumerarTelefonosClientes(clientes) {
  ...
  for (const c of clientes || []) {
    ...
    for (const ct of c.contacts || []) {
      ...
      // persona: el Cel se enumera con su mismo persona/rol, nunca aparte.
      agregar(ct.phone, contacto);
      agregar(ct.phone2, contacto);
      agregar(ct.fax, contacto);
    }
    for (const b of c.branches || []) {
      ...
      // mismo contacto que su Telefono.
      agregar(b.phone, domicilio);
      agregar(b.fax, domicilio);
    }
  }
  return entradas;
}
