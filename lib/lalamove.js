...
import {
  encabezadosLalamove, serviciosDeMexico, vehiculosQueAlcanzan,
...
export async function tarifasLalamove({ cp, pesoKg }) {
  ...
  let vehiculos;
  try {
    vehiculos = vehiculosQueAlcanzan(await servicios(), pesoKg);
  } catch (err) {
    return { rates: [], warnings: [`Lalamove no disponible: ${err.message}`] };
  }
  if (!vehiculos.length) {
    return { rates: [], warnings: [`Lalamove: ${pesoKg} kg excede la carga de sus vehiculos`] };
  }
