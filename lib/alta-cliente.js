...
export async function upgradeFiscal(id, datosFiscales, deps = {}) {
  ...
  if (verificacion.estado === 'otro') {
    ...
    // que no se escribio nada y cual es su siguiente paso. "Fusion" es jerga del
    // modulo y se queda en el detalle.
    const quien = verificacion.dueno?.CustName ? `${verificacion.dueno.CustName} (${duenoId})` : `${duenoId}`;
    const mensaje = `Este RFC ya es del Cliente Operam ${quien}. No se cambio nada. Si es la misma empresa, cotiza sobre ese Cliente Operam; si hay dos cuentas del mismo contribuyente, hay que unificarlas a mano en Operam.`;
