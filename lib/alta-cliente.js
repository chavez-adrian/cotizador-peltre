...
import {
  ...
  diffBranchDomicilio, CAMPOS_BRANCH_DEL_CLIENTE, resolverSalesTypeId, FUENTE_ALTA_GENERICA, FUENTE_SUCURSAL_CREADA,
...
import { releerContactosDomicilioTrasEscribir } from './contactos-domicilio-io.js';
import { DIFF_FISCAL_CAMPOS, leerValorOperam, mensajeRfcDeOtroClienteOperam, SALIDAS_DEDUP } from '../public/js/alta-logica.js';
...
//
// --- Resultado --------------------------------------------------------------
//   { tipo: 'lograda',  clienteId, domicilioId, creadoNuevo, logId, pasos }
...
//   de contactos-domicilio-io: releerContactosDomicilioTrasEscribir (#397)
//
...
function deOperam(deps) {
  return {
    ...
    releerContactosDomicilioTrasEscribir: deps.releerContactosDomicilioTrasEscribir || releerContactosDomicilioTrasEscribir,
  };
}
...
async function poolDedup(solicitud, io) {
  ...
  return {
    rfc, nombre, padron,
    dedup: marcarMismoRfcReal(agregarCandidatosPorCustRef(dedup, padron, c.nombreCorto), rfcReal),
  };
}

// La marca del mismo contribuyente (#377), puesta AL FINAL y por COMPARACION del
// RFC, no por el camino que trajo al candidato: el dueno del nombre corto sale
// del padron completo sin filtrar por RFC (#242) y es justamente el unico camino
// que descubre que el cliente ya existia bajo un RFC real, asi que derivarla del
// veredicto 'exacto' dejaba fuera al candidato que mas importa. Sin RFC real
// capturado no marca a nadie: el generico lo comparten por diseno (ADR-0001) y no
// identifica.
function marcarMismoRfcReal(dedup, rfcReal) {
  if (!rfcReal || dedup?.tipo !== 'candidatos') return dedup;
  return {
    ...dedup,
    candidatos: (dedup.candidatos || []).map(k => (normalizarRfc(k?.tax_id) === rfcReal ? { ...k, _rfcIgual: true } : k)),
  };
}
...
// El candidato que coincide por RFC real EXACTO, si lo hay (#377). Es el mismo
// contribuyente: la marca la pone poolDedup sobre el veredicto 'exacto' de la
// dedup, y ningun otro camino la inventa (un choque de nombre corto o de nombre
// puede tener otro RFC, o el generico, y ahi si es un parecido).
function duenoDelMismoRfc(dedup) {
  if (dedup?.tipo !== 'candidatos') return null;
  return (dedup.candidatos || []).find(k => k?._rfcIgual === true) || null;
}

// Las salidas de la pregunta dependen del MOTIVO del candidato (#377): con el
// mismo RFC real "ninguno es el mismo" no existe -- crearia una segunda cuenta
// del mismo contribuyente, que es justo lo que la dedup viene a evitar y lo que
// el upgrade fiscal ya frena con su gate de fusion. Por nombre corto o por
// parecido de nombre las tres salidas siguen, porque ahi el vendedor si puede
// saber que son clientes distintos.
const SALIDAS_MISMO_RFC = SALIDAS_DEDUP.filter(s => s !== 'ninguno');
const NOTA_MISMO_RFC = 'No se puede crear otro Cliente Operam con este mismo RFC: es el mismo contribuyente.';

// La pregunta al vendedor con sus salidas (CONTEXT.md "Deduplicacion de
...
// menos botones segun por donde entro. QUE salidas hay lo decide el motivo del
// candidato (#377) y no la pantalla: el mensaje dice por que falta una, porque
// un boton menos sin explicacion se lee como una falla de la pagina.
function preguntaCandidatos({ pasos, mensaje, detalle, candidatos, ctx }) {
  const mismoRfc = (candidatos || []).some(k => k?._rfcIgual === true);
  return {
    tipo: 'pregunta', motivo: 'candidatos', pasos,
    mensaje: mismoRfc ? `${mensaje}. ${NOTA_MISMO_RFC}` : mensaje,
    detalle,
    opciones: mismoRfc ? SALIDAS_MISMO_RFC : SALIDAS_DEDUP,
...
// La cartera del Cliente Operam (#414): de quien es el cliente segun los
// domicilios de entrega que YA tiene. En Operam el vendedor vive en el domicilio
// y el "Reporte de Comisiones Por Flujo" paga por domicilio (ni el quote ni la
// factura tienen vendedor propio), asi que el domicilio NUEVO de un cliente
// existente escrito a nombre de quien lo captura partia la cartera en silencio.
// Regla (a') de Adrian (2026-09-22): si los domicilios ACTIVOS (`inactive`
// distinto de "1") son de UNA sola persona, esa persona es el vendedor del
// domicilio nuevo; sin ninguna, o con varias, no hay a quien heredar.
// Persona = su id de Operam esta en el registro de vendedores (`operam_id`): los
// canales (Amazon, Shopify, Mostrador, Mercado Libre), el vacio y el 0 no lo
// estan. Se compara por ID y no por nombre: GET /branches/:code trae el id
// `salesman` (llaves medidas en vivo el 2026-09-07 sobre el branch 564), mientras
// que GET /customers/:id solo trae `salesman_name`, con acentos que el registro
// no siempre escribe igual. Un vendedor del registro sin operam_id nunca empata
// con un domicilio vacio (la misma trampa de mapearSalesman, #76).
//   -> { salesman, motivo: 'persona-unica' | 'varias-personas' | 'sin-persona',
//        persona, activos: [{ branch_code, salesman, persona }], inactivos }
export function vendedorDeCartera(domicilios, vendedores) {
  const personas = new Map();
  const activos = [];
  const inactivos = [];
  for (const d of domicilios || []) {
    if (String(d?.inactive ?? '') === '1') {
      inactivos.push(d?.branch_code);
      continue;
    }
    const id = String(d?.salesman ?? '').trim();
    const persona = id
      ? (vendedores || []).find(v => v?.operam_id != null && String(v.operam_id) === id)
      : null;
    activos.push({ branch_code: d?.branch_code, salesman: id, persona: persona ? persona.name : null });
    if (persona) personas.set(String(persona.operam_id), persona);
  }
  const base = { activos, inactivos };
  if (!personas.size) return { salesman: null, motivo: 'sin-persona', persona: null, ...base };
  if (personas.size > 1) return { salesman: null, motivo: 'varias-personas', persona: null, ...base };
  const [unica] = personas.values();
  return { salesman: unica.operam_id, motivo: 'persona-unica', persona: unica, ...base };
}

// El vendedor del domicilio NUEVO de un Cliente Operam existente (#414) y el paso
// que lo dice, en dos capas. `pedido` es el `salesman` de la solicitud (el selector
// del alta completa, o el vendedor de la cotizacion al subirla). Con persona unica
// en la cartera gana ella aunque la solicitud diga otra cosa; sin ella se escribe
// el pedido, como antes de #414, pero el paso lo avisa. Nunca bloquea. Solo corre
// sobre un Cliente Operam ELEGIDO ("es otro domicilio de este cliente"): el que
// nace en esta misma corrida no tiene cartera y su domicilio sigue saliendo con
// el vendedor de la solicitud.
async function carteraDelDomicilioNuevo({ previas, pedido, clienteId, io }) {
  const name = 'vendedor branch';
  const pidio = pedido == null || pedido === '' ? '(ninguno)' : pedido;
  let vendedores;
  try {
    vendedores = await io.listar();
  } catch (err) {
    return {
      salesman: pedido,
      paso: {
        name, status: 'warn',
        mensaje: 'El domicilio nuevo queda con el vendedor que se pidio: no se pudo revisar quien atiende a este cliente',
        detalle: `registro de vendedores: ${err?.message ?? err}; se escribe el salesman de la solicitud, ${pidio}`,
      },
    };
  }
  const r = vendedorDeCartera(previas, vendedores);
  if (r.motivo === 'persona-unica') {
    const origen = r.activos.filter(d => d.persona).map(d => d.branch_code).join(', ');
    return {
      salesman: r.salesman,
      paso: {
        name, status: 'ok',
        mensaje: `El domicilio nuevo queda a nombre de ${r.persona.name}, que atiende a este cliente`,
        detalle: `salesman ${r.salesman} de los domicilios activos ${origen} del cliente ${clienteId}; la solicitud pedia ${pidio}`,
      },
    };
  }
  const nombre = (vendedores || []).find(v => v?.operam_id != null && String(v.operam_id) === String(pedido ?? ''))?.name;
  const aNombreDe = nombre ? `a nombre de ${nombre}` : pidio !== '(ninguno)' ? `a nombre del vendedor ${pidio} de Operam` : 'sin vendedor';
  const encontrados = r.activos
    .map(d => `${d.branch_code}=${d.salesman || '(vacio)'} ${d.persona || '(fuera del registro de vendedores)'}`)
    .join(', ') || 'ninguno';
  const inactivos = r.inactivos.length ? `; inactivos, no cuentan: ${r.inactivos.join(', ')}` : '';
  return {
    salesman: pedido,
    paso: {
      name, status: 'warn',
      mensaje: `El domicilio nuevo queda ${aNombreDe}: el cliente no tiene un vendedor unico en sus domicilios`,
      detalle: `${r.motivo} en el cliente ${clienteId}: domicilios activos ${encontrados}${inactivos}; se escribe el salesman de la solicitud, ${pidio}`,
    },
  };
}
...
async function altaSinLock(solicitud, deps = {}) {
  ...
  // La fila de auditoria del cliente RECIEN CREADO (#356): quien respalda la
  // constancia en Dropbox la corrige cuando la subida resuelve, y por eso
  // dropbox_ok deja de ser siempre null. Es una promesa a proposito -- el alta
  // no espera a la auditoria, igual que antes.
  let logIdCreado = null;
  ...
  // #121 impone.
  let usoCfdiDelPost;
  // El IdCIF de la constancia, mismo quirk (#391): el POST lo ignora y el PUT lo
  // guarda (medido en vivo 2026-09-25). Vacio en el alta sin constancia.
  let idcifDelPost;
  ...
  try {
    ...
    if (clienteId != null) {
      ...
      // "Ninguno es el mismo" contra el MISMO RFC real (#377): esa salida no se
      // ofrecio, asi que llegar aqui es un cuerpo manipulado o el reintento de una
      // pregunta vieja. Se bloquea con la misma explicacion que la fusion del
      // upgrade fiscal (#375), cero escrituras, y el dueno viaja nombrado: el
      // vendedor tiene que cotizar sobre ese Cliente Operam o unificar a mano. El
      // clienteId NO viaja -- quien pide el alta lo persiste para el reintento, y
      // eso ligaria la operacion a un cliente que el vendedor dijo que no era.
      const mismoRfc = duenoDelMismoRfc(dedup);
      if (mismoRfc) {
        const dueno = { cliente_id: mismoRfc.customer_id, nombre: mismoRfc.CustName || `el Cliente Operam ${mismoRfc.customer_id}` };
        const mensaje = mensajeRfcDeOtroClienteOperam({ cliente_id: mismoRfc.customer_id, nombre: mismoRfc.CustName });
        const detalle = `RFC ${rfc} del Cliente Operam ${mismoRfc.customer_id}: fusion manual, la API no unifica cuentas`;
        io.logCliente(rfc, nombre, 'fusion-bloqueada', mismoRfc.customer_id, fuenteAuditoria, null, null);
        pasos.push({ name: 'dedup', status: 'error', mensaje, detalle });
        return { tipo: 'bloqueo', motivo: 'fusion', pasos, mensaje, detalle, dueno };
      }
      ...
      const cuerpoPost = buildClienteBody(clienteNuevo);
      usoCfdiDelPost = cuerpoPost.timbrado_uso_cfdi;
      idcifDelPost = cuerpoPost.idcif;
      let creado;
      try {
      ...
      logIdCreado = io.logCliente(rfc, nombre, forzado ? 'creado-forzado' : 'creado', clienteId, fuenteAuditoria, null,
    ...
    if (crearDomicilio && domicilioId == null) {
      ...
      try {
        ...
        if (yaCreada) {
          ...
          // La cartera se decide SOLO al crear (#414): el equivalente de un intento
          // anterior ya existe y no se vuelve a escribir, y la busqueda de arriba no
          // mira al vendedor, asi que un reintento sigue encontrando el mismo.
          const cartera = await carteraDelDomicilioNuevo({ previas, pedido: salesman, clienteId, io });
          pasos.push(cartera.paso);
          const datosConCartera = cartera.salesman != null && cartera.salesman !== ''
            ? { ...datosDomicilio, salesman: cartera.salesman }
            : datosDomicilio;
          creada = await io.crearBranchCliente(clienteId, datosConCartera);
    ...
    // El IdCIF de la constancia viaja aqui tambien (#391) y, como ninguna lectura
    // lo devuelve (#373), su unica verificacion es el eco de este PUT.
    if (creadoNuevo) {
      const dimensiones = {
        dimension_id: 1, dimension2_id: 5, timbrado_uso_cfdi: usoCfdiDelPost,
        ...(idcifDelPost ? { idcif: idcifDelPost } : {}),
      };
      const que = idcifDelPost
        ? 'la clasificacion interna, el uso de CFDI y el IdCIF (SAT)'
        : 'la clasificacion interna y el uso de CFDI';
      try {
        const eco = await io.actualizarClienteDirecto(clienteId, dimensiones);
        const [idcifIgnorado] = idcifDelPost
          ? camposNoAplicados({ idcif: { label: 'IdCIF (SAT)', nuevo: idcifDelPost } }, eco)
          : [];
        pasos.push(idcifIgnorado
          ? {
            // Sin valor anterior: la API no lo relee y el POST pudo o no
            // guardarlo, asi que afirmar con cual se quedo seria inventarlo.
            name: 'PUT customer (dimensiones)', status: 'warn',
            mensaje: `Operam no guardo el IdCIF (SAT) ${idcifDelPost} de la constancia: capturalo en la ficha del Cliente Operam`,
            detalle: `PUT /customers/${clienteId} campo idcif: ${idcifIgnorado.motivo}`,
          }
          : {
            name: 'PUT customer (dimensiones)', status: 'ok',
            mensaje: `Se guardaron ${que} del Cliente Operam`,
        ...
        pasos.push({
          ...
          mensaje: `No se guardaron ${que} del Cliente Operam`,
          detalle: `PUT /customers/${clienteId}: ${err.message}`,
        });
      }
    } else {
      pasos.push({
        ...
        detalle: `cliente ${clienteId} conserva dimension_id/dimension2_id/timbrado_uso_cfdi`,
      });
    }
    ...
    const datosDomicilio = buildBranchGenerico(c, { salesman });
    if (creadoNuevo) {
      // El PUT EXIGE br_ref (#386): sin el Operam responde 406 "La referencia de
      // sucursal es requerida" y el domicilio capturado no se aplica -- el unico
      // aviso era la fila en error del panel. El paso Envio no captura referencia
      // corta, asi que se relee la que Operam auto-creo junto con el cliente y se
      // reenvia tal cual: el PUT es REPLACE, y omitirla (#96) no la conservaba,
      // la hacia rebotar. La capturada en el alta completa manda sobre la releida;
      // si la relectura no trae ninguna, actualizarBranchCliente cae a br_name.
      if (!datosDomicilio.br_ref) {
        try {
          const previo = await io.obtenerBranch(domicilioId);
          if (previo?.branch_ref) datosDomicilio.br_ref = previo.branch_ref;
        } catch {
          // Sin relectura el PUT sale igual, con la referencia de respaldo.
        }
      }
      try {
    ...
    // El telefono y el correo del domicilio NO vienen en GET /branches/:code
    // (#431): se leen de branches[] de GET /customers/:id, la MISMA lectura que
    // la verificacion del Cel hace abajo, asi que se lee una vez y se reusa. Si
    // no se puede leer, esos campos salen de la comparacion (no se afirma que
    // Operam los ignoro) y el detalle dice que quedaron sin comprobar.
    let clienteReleido = null;
    const releerCliente = async () => {
      if (!clienteReleido) clienteReleido = await io.obtenerClientePorId(clienteId);
      return clienteReleido;
    };
    if (creadoNuevo) {
      try {
        const fresco = await io.obtenerBranch(domicilioId);
        const delCliente = CAMPOS_BRANCH_DEL_CLIENTE.filter(campo => String(datosDomicilio[campo] ?? '').trim());
        let branchDelCliente = null;
        let sinComprobar = '';
        if (delCliente.length) {
          try {
            branchDelCliente = ((await releerCliente())?.branches || [])
              .find(b => b && String(b.branch_code) === String(domicilioId)) || null;
            if (!branchDelCliente) sinComprobar = `el domicilio ${domicilioId} no aparece en GET /customers/${clienteId}`;
          } catch (err) {
            sinComprobar = `GET /customers/${clienteId}: ${err.message}`;
          }
        }
        const camposNoActualizados = diffBranchDomicilio(fresco, datosDomicilio, branchDelCliente);
        const lecturas = `GET /branches/${domicilioId}${branchDelCliente ? ` y GET /customers/${clienteId}` : ''}`;
        const nota = sinComprobar ? `; ${delCliente.join(', ')} sin comprobar (${sinComprobar})` : '';
        pasos.push(camposNoActualizados.length
          ? {
            ...
            detalle: `${lecturas}: Operam ignoro ${camposNoActualizados.map(x => x.campo).join(', ')}${nota}`,
            camposNoActualizados,
          }
          : {
            name: 'verificar branch', status: 'ok',
            mensaje: 'El domicilio de entrega quedo guardado en Operam',
            detalle: `${lecturas}${nota}`,
          });
      } catch (err) {
        pasos.push({
    ...
    if (celContacto && (creadoNuevo || domicilioEscritoAqui)) {
      try {
        const fresco = await releerCliente();
    ...
    // Padron de contactos de domicilio tras escribir un domicilio de entrega (#397):
    // sus Contactos en Operam solo salen de contact_list, que no filtra, asi que no
    // hay entrada puntual que meterle -- se relee entero, en segundo plano y a su
    // ritmo (lib/contactos-domicilio-io.js). Si ya habia un barrido en vuelo, que
    // pudo haber leido la pagina de este domicilio antes de escribirlo, se agenda
    // una relectura mas al terminar ese barrido.
    if (domicilioEscritoAqui) {
      try {
        Promise.resolve(io.releerContactosDomicilioTrasEscribir())
          .catch(err => console.warn('[alta-cliente] el refresco de contactos de domicilio fallo:', err?.message));
      } catch (err) {
        console.warn('[alta-cliente] el refresco de contactos de domicilio fallo:', err?.message);
      }
    }

    const segmentoDiferido = await pasoSegmento(s, { io, pasos, clienteId, rfc: rfcGenericoDe(c), nombre: nombreDeAlta(s) });

    return { tipo: 'lograda', clienteId, domicilioId, creadoNuevo, logId: logIdCreado, pasos, ...(segmentoDiferido ? { segmentoDiferido } : {}) };
  } catch (err) {
    return {
...
//
// Resultado:
//   { tipo: 'lograda', clienteId, eco, logId, camposNoAplicados, segmento, pasos }
// `camposNoAplicados` son los que Operam recibio y NO guardo -- jamas los que
// este modulo decidio no mandarle (#379): esos ya salieron como paso propio.
...
export async function upgradeFiscal(id, datosFiscales, deps = {}) {
  ...
  if (verificacion.estado === 'otro') {
    const duenoId = verificacion.dueno?.cliente_id;
    const nombre = verificacion.dueno?.CustName || `el Cliente Operam ${duenoId}`;
    // Mensaje en dos capas (#375): el vendedor necesita saber de quien es el RFC,
    // que no se escribio nada y cual es su siguiente paso. La redaccion es del
    // nucleo compartido con el navegador; "fusion" se queda en el detalle.
    const mensaje = mensajeRfcDeOtroClienteOperam({ cliente_id: duenoId, nombre: verificacion.dueno?.CustName });
    const detalle = `RFC ${rfc} del Cliente Operam ${duenoId}: fusion manual, la API no unifica cuentas`;
    io.logCliente(rfc, razonSocial, 'fusion-bloqueada', duenoId, FUENTE_CSF_UPGRADE, null, null);
    pasos.push({ name: 'gate RFC', status: 'error', mensaje, detalle });
    return {
      tipo: 'bloqueo', motivo: 'fusion', pasos, mensaje, detalle,
      dueno: { cliente_id: duenoId, nombre },
    };
  }
  pasos.push({
  ...
  // #379: los campos que este modulo decidio NO mandar, o que ya se reportaron con
  // su motivo real, salen de la verificacion post-PUT. Un campo que nunca viajo no
  // lo "ignoro Operam": reportarlo asi le pide al vendedor corregir algo que esta
  // bien. Una sola lista para que el siguiente campo omitido no repita el error.
  // Las llaves son las de LECTURA (`operam` de DIFF_FISCAL_CAMPOS, que es lo que
  // trae el diff), NUNCA las del payload: no son las mismas (#169).
  const camposFueraDeLaVerificacion = new Set();
  ...
  if ('cust_ref' in payload) {
    ...
    if (vacio || propio) {
      ...
      camposFueraDeLaVerificacion.add('cust_ref');
      pasos.push({
  ...
  if (datos.segmentoId) {
    ...
    if (r.ok) {
      ...
      // Ya se dice aqui, con el motivo real: la API v3 nunca escribe el segmento,
      // asi que repetirlo como campo ignorado por el PUT seria falso.
      camposFueraDeLaVerificacion.add('segmento_id');
      pasos.push({
        name: 'segmento', status: 'error',
  ...
  try {
    ...
    camposPendientes = camposNoAplicados(calcularDiffFiscal(fresco, datos), eco)
      // El diff sigue viendo distintos los campos que el PUT no llevo: sin este
      // filtro, lo que se omitio a proposito se le reporta al vendedor como si
      // Operam lo hubiera ignorado (#379).
      .filter(c => !camposFueraDeLaVerificacion.has(c.campo))
  ...
  // La fila de esta actualizacion (#356): el endpoint la corrige con el
  // resultado de la subida de la constancia, igual que en el alta.
  const logId = io.logCliente(rfc, razonSocial, 'actualizado', id, FUENTE_CSF_UPGRADE, null,
    pasos.some(p => p.name === 'verificar fiscal' && p.status === 'error')
      ? 'La verificacion post-PUT fallo (el PUT si se aplico)'
      : null);

  return {
    tipo: 'lograda', clienteId: id, eco, logId,
