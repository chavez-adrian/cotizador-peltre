...
import { DIFF_FISCAL_CAMPOS, leerValorOperam, mensajeRfcDeOtroClienteOperam } from '../public/js/alta-logica.js';
...
async function poolDedup(solicitud, io) {
  ...
  // El match exacto se normaliza a candidato, pero MARCADO (#377): coincidir por
  // RFC real exacto no es parecerse, es ser el mismo contribuyente, y de esa
  // marca dependen las salidas que la pregunta ofrece.
  const dedup = crudo.tipo === 'exacto'
    ? { tipo: 'candidatos', candidatos: [{ ...crudo.cliente, _rfcIgual: true }] }
    : crudo;
...
const SALIDAS_MISMO_RFC = ['usar', 'otro-domicilio'];
