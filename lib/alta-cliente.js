...
import {
  ...
  actualizarBranchCliente, listarSalesTypes, verificarRfcLibre,
...
//   de indice-telefonos: refrescarIndice          (upgrade fiscal, #367)
//
...
export async function upgradeFiscal(id, datosFiscales, deps = {}) {
  ...
  if ('cust_ref' in payload && !(clienteAntes && nombreCortoSustituible(clienteAntes))) {
    delete payload.cust_ref;
    pasos.push({
      name: 'nombre corto', status: 'omitido',
      mensaje: 'El Cliente Operam ya tiene nombre corto propio: se conserva el que tenia',
