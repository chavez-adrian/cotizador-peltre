...
// Configuracion comercial que el PUT de cliente SI persiste, con la llave con la
// que se ESCRIBE y la que hay que leer para compararla (#169: no son la misma).
...
const CAMPOS_COMERCIALES = [
  { campo: 'sales_type', leer: k => k?.sales_type, que: 'la lista de precios' },
  { campo: 'segmento_id', leer: k => k?.segmento?.id, que: 'el segmento' },
  { campo: 'timbrado_uso_cfdi', leer: k => k?.timbrado_uso_cfdi, que: 'el uso de CFDI' },
];
...
async function pasoConfiguracionComercial(clienteId, creadoNuevo, comercial, io) {
  ...
  const cambios = {};
  const cambiado = [];
  for (const { campo, leer, que } of CAMPOS_COMERCIALES) {
    const nuevo = capturado[campo];
    if (nuevo === '' || nuevo == null) continue;
    if (String(leer(actual) ?? '') === String(nuevo)) continue;
    cambios[campo] = nuevo;
    cambiado.push(que);
  }
  if (!cambiado.length) {
