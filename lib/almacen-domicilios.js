...
// Operam manda el nombre del domicilio con entidades HTML (`D&#039;ANGELO`,
// `FILM &amp; TAPE`; #446). Se decodifica UNA vez, en una sola pasada (asi
// `&amp;#039;` queda como `&#039;` y no como apostrofe), y el resultado es TEXTO:
// el panel lo sigue escapando al pintarlo.
const ENTIDADES = { amp: '&', lt: '<', gt: '>', quot: '"', apos: "'" };

function decodificarEntidades(s) {
  return s.replace(/&(#x[0-9a-f]+|#\d+|[a-z]+);/gi, (m, e) => {
    if (e[0] !== '#') return Object.hasOwn(ENTIDADES, e) ? ENTIDADES[e] : m;
    const codigo = e[1] === 'x' || e[1] === 'X' ? parseInt(e.slice(2), 16) : parseInt(e.slice(1), 10);
    return codigo > 0 && codigo <= 0x10ffff ? String.fromCodePoint(codigo) : m;
  });
}
  ...
  for (const c of clientes || []) {
    ...
    for (const b of c?.branches || []) {
      ...
      const fila = {
        ...
        branchCode,
        domicilio: decodificarEntidades(texto(detalle?.br_name) || texto(b.br_name)),
      };
