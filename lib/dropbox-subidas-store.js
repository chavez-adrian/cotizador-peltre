...
// sandbox de la app (las dos filas decian "Subido"). Una subida al sandbox lo
// guarda en null. Las subidas son fire-and-forget en los tres flujos del repo
...
// La tabla ya existe en produccion desde #356, asi que a ella la columna de
// #399 le entra aparte. Las filas anteriores quedan en NULL y su lugar se
// infiere por la ruta (lib/dropbox-destinos.js#lugarDeSubida).
const SCHEMA_NAMESPACE = 'ALTER TABLE dropbox_subidas ADD COLUMN IF NOT EXISTS namespace TEXT';

let schemaListo = null;
async function ensureSchema() {
  // Un ALTER que falle (lock, timeout) no puede quedar cacheado: el INSERT ya
  // nombra la columna y el registro se quedaria mudo hasta el siguiente deploy.
  if (!schemaListo) {
    schemaListo = query(SCHEMA).then(() => query(SCHEMA_NAMESPACE)).catch(err => {
      schemaListo = null;
      throw err;
    });
  }
