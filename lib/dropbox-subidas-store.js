...
// error y momento. Las subidas son fire-and-forget en los tres flujos del repo
...
export async function registrar({ flujo, destino, ok, error } = {}) {
  try {
    await ensureSchema();
    const subida = {
    ...
    const r = await query(
      'INSERT INTO dropbox_subidas (flujo, destino, archivo, ok, error_msg) VALUES ($1,$2,$3,$4,$5)',
      [subida.flujo, subida.destino, subida.archivo, subida.ok, subida.error]
    );
    if (r === null) {
...
export async function listarRecientes(limite = 100) {
  try {
    ...
    if (r === null) return leerJson().slice(-limite).reverse();
