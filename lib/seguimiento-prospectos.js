...
// `faltaCotizarIds` son las Oportunidades que llevan la etiqueta "Ya tiene
// Cliente Operam, falta cotizar" (#400): el juicio es de `faltaCotizar`
// (lib/tabla-prospectos.js) y necesita las cotizaciones que quien pregunta
// PUEDE VER, asi que lo arma la ruta y aqui solo se transporta -- la misma
// division que con las tarjetas sin dueno de la cola Hoy. Sin ellas ningun item
// la inventa desde la liga: eso es exactamente lo que pintaba "falta cotizar"
// sobre un Contacto que ya habia cotizado.
export function calcularColaProspectos(prospectos, ahora, faltaCotizarIds = new Set()) {
  const cola = [];
  for (const p of prospectos || []) {
    ...
    cola.push({
      ...
      etapa: p.etapa,
      vendedor: p.vendedor,
      // Fecha de CAPTURA y empresa (#289): el buscador de Prospectos y el de
      // Hoy filtran la cola con el mismo criterio que la lista, y sin estas dos
      // el item no alcanza a contestar "de la ultima semana" ni "Hotel Azul".
      fecha: p.fecha,
      empresa: (p.data && p.data.empresa) || null,
      horas,
      toques: toques.length,
      color: semaforo(horas, p.canal),
      sugerirNoUtil: toques.length >= SUGERIR_NO_UTIL_TOQUES,
      // Contacto ya ligado a un Cliente Operam que todavia no cotiza (#46/#400,
      // CONTEXT.md "Ya tiene Cliente Operam, falta cotizar"): sigue en la cola
      // hasta Cotizado o No util, con etiqueta visible en la UI.
      faltaCotizar: faltaCotizarIds.has(p.id),
