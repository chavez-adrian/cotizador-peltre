...
// Origen "Cliente Actual" retirado (issue #341, ADR-0016): el mismo celular que
// vuelve a comprar no llega por ninguna puerta, asi que ese valor desaparecio
// del catalogo. Los prospectos ya guardados con el valor viejo se leen como
// "Relacion existente" -- el Contacto nuevo que llega por una relacion
// comercial que ya tenemos. Idempotente: un canal ajeno al valor viejo se
// devuelve igual.
const CANAL_VIEJO_A_NUEVO = {
  'Cliente Actual': 'Relación existente',
};

export function canalMigrado(canalViejo) {
  return CANAL_VIEJO_A_NUEVO[canalViejo] || canalViejo;
}
...
export function migrarProspecto(p) {
  return { ...p, etapa: etapaProspectoMigrada(p.etapa), canal: canalMigrado(p.canal) };
}
