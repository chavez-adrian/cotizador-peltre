...
// PARIDAD EN TRES CAPAS con el formato oficial de Operam (#326). La
// especificacion de referencia es lib/pdf-generator.js, ya calcado con la
// geometria medida del PDF de Operam; aqui se TRADUCE a CSS fluido, no se
// re-mide el PDF, y la duplicacion de layout entre los dos generadores es
// deliberada (coordenadas absolutas alla, flujo CSS aca).
//   1. Contenido y semantica: paridad EXACTA -- las mismas etiquetas con la
//      ortografia medida (el numero y el codigo llevan acento; y NO lo llevan
//      donde Operam no lo trae: "COTIZACION", "Valido hasta"), mismo orden de
//      secciones e importes a 2 decimales de PRESENTACION (la matematica
//      interna sigue a precision completa: el documento imprime el total que
//      recibe, nunca la suma de lo ya redondeado).
//   2. Escritorio e impresion: fidelidad alta -- colores medidos
//      (#CCCCCC titulo, #FF0000 folio, #808080 separadores, #DDDDDD cajas),
//      CERO lineas verticales (regla negra bajo los encabezados, separadores
//      grises entre filas), las dos cajas de solo trazo, Helvetica y las
//      medidas del PDF traducidas a 96 dpi (20pt = 26.7px, 7.5pt = 10px).
//   3. Telefono (<=480px): manda la LEGIBILIDAD -- se conserva el reflow de
//      tarjetas de #302 y solo se alinea el lenguaje visual. No se persigue
//      pixel-perfect: por eso las diferencias de esta capa son deliberadas.
// Lo que el formato de Operam no tiene y el cotizador SI conserva: titulo
// PRE-COTIZACION sin folio (ADR-0009), fotos opcionales, barra de impresion
// de la liga y datos bancarios reales en el pie.
function fmt(n) {
...
export function generateQuoteHTML(data, options = {}) {
  ...
  // Misma composicion que el delivery_address del quote (#332): el documento decia
  // "Bosques de Europa 163, 4, ..." donde el quote 1283 decia "163 Int. 4, ...".
  const dirEntrega = domicilioEntrega(c);

  const hasImages = incluirFotos && Object.keys(images).length > 0;
  const headerImgCell = hasImages ? '<th class="th-img">Foto</th>' : '';
  // Anchos medidos del formato oficial (545.8pt utiles): 54.6pt por columna
  // y 218.3pt para Descripcion = 10% y 40%. La columna Foto mide lo mismo que
  // las demas y el ancho se lo quita a Descripcion, igual que en el PDF.
  const anchosItems = hasImages ? [10, 10, 30, 10, 10, 10, 10, 10] : [10, 40, 10, 10, 10, 10, 10];
  const colgroupItems = `<colgroup>${anchosItems.map((w) => `<col style="width:${w}%">`).join('')}</colgroup>`;
...
<title>${folio ? 'Cotizaci&oacute;n ' + folio : 'Pre-cotizacion'} - Peltre Nacional</title>
<style>
* { box-sizing: border-box; margin: 0; padding: 0; }
body { font-family: Helvetica, Arial, sans-serif; font-size: 10px; color: #000000; background: #fff; }
...
.page { max-width: 794px; margin: 24px auto; background: #fff; border-radius: 4px; box-shadow: 0 2px 12px rgba(0,0,0,0.10); padding: 30px; }

/* Header 3 columns */
.header { display: grid; grid-template-columns: 1fr 1fr 1fr; align-items: flex-start; padding-bottom: 10px; border-bottom: 1px solid #000000; margin-bottom: 9px; gap: 12px; }
.company-name { font-size: 14px; font-weight: 700; margin-bottom: 4px; }
.company-info { font-size: 9.7px; color: #000000; line-height: 1.3; text-align: left; }
.quote-block { text-align: right; }
.quote-block h2 { font-size: 26.7px; font-weight: 700; color: #CCCCCC; letter-spacing: 1px; line-height: 1; margin-bottom: 3px; }
.quote-num { color: #FF0000; font-weight: 700; }
.quote-meta { font-size: 9.7px; color: #000000; }
...
.page-num { font-size: 8px; color: #000000; text-align: right; }

/* Client */
.client-section { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; padding-bottom: 8px; border-bottom: 1px solid #000000; margin-bottom: 16px; }
.section-title { font-size: 9.7px; font-weight: 400; color: #000000; margin-bottom: 2px; }
.client-block p { font-size: 9.7px; line-height: 1.3; color: #000000; }
.client-label { font-weight: 700; color: #000000; }

/* Tabla de referencia, dentro de su caja de solo trazo gris (#326): el formato
   oficial no dibuja ninguna linea vertical -- las columnas las define la regla
   negra bajo los encabezados y el separador gris del fondo de la fila. */
.ref-box { border: 1px solid #DDDDDD; margin-bottom: 14px; }
.comercial-table { width: 100%; border-collapse: collapse; font-size: 10px; }
.comercial-table th { color: #000000; font-weight: 700; padding: 4px 7px; border-bottom: 1px solid #000000; text-align: center; font-size: 10px; }
.comercial-table td { padding: 4px 7px; border-bottom: 1px solid #808080; font-size: 10px; text-align: center; }

/* Payment terms */
.pago-line { font-size: 10px; margin-top: 0; margin-bottom: 18px; }

/* Caja de partidas: el mismo trazo gris del formato oficial, envolviendo tabla,
   notas y totales (#326) */
.products-box { border: 1px solid #DDDDDD; margin-bottom: 20px; }

/* Products table */
table.products { width: 100%; border-collapse: collapse; font-size: 10px; }
table.products th { color: #000000; font-size: 10px; font-weight: 700; padding: 6px 7px; text-align: center; border-bottom: 1px solid #000000; }
table.products th.num { text-align: center; }
table.products td { padding: 7px 7px; border-bottom: 1px solid #808080; vertical-align: middle; text-align: center; }
table.products td.num { text-align: right; }
table.products td.td-code { font-size: 10px; color: #000000; word-wrap: break-word; }
...
.notes-block { flex: 1; }
.notes-block ul { list-style: none; padding-left: 4px; }
.notes-block li { font-size: 10px; color: #000000; line-height: 1.5; }
.notes-block li::before { content: "- "; }

.totals-table { min-width: 240px; border-collapse: collapse; font-size: 10px; }
.totals-table td { padding: 4px 9px; }
.totals-table td:last-child { text-align: right; font-weight: 400; min-width: 100px; }
.totals-table tr.total-row td { font-weight: 700; padding-top: 6px; }

/* Footer */
.footer { padding-top: 3px; font-size: 10px; color: #000000; text-align: left; line-height: 1.3; }

@media print {
  @page { size: A4; margin: 8mm; }
  body { background: #fff; }
  .print-bar { display: none !important; }
  .page { max-width: none; margin: 0; box-shadow: none; border-radius: 0; padding: 0; }
}
...
@media screen and (max-width: 480px) {
  ...
  /* Tabla comercial: de 5 columnas a lista de pares etiqueta:valor. La caja de
     trazo gris no aporta nada cuando la tabla ya es una lista (#326). */
  .ref-box { border: none; }
  ...
  .comercial-table td { border: none; border-bottom: 1px solid #DDDDDD; padding: 5px 0; font-size: 13px; text-align: left; }
  ...
  table.products td { display: block; border: none; padding: 0; font-size: 14px; text-align: left; }
  table.products td.td-img { margin-bottom: 8px; }
  table.products td.td-img .product-img { width: 64px; height: 64px; }
  table.products td.td-code { font-weight: 700; font-size: 13px; color: #000000; }
  ...
  table.products td[data-field="total"] strong { color: #FF0000; }
...
<div class="print-bar">
  <span>${folio ? 'Cotizaci&oacute;n ' + esc(folio) + ' Peltre Nacional' : 'Pre-cotizacion Peltre Nacional'}</span>
  ${id != null ? `<a class="btn-print" href="/api/cotizacion/pdf/${esc(id)}" target="_blank" rel="noopener">Ver PDF</a>` : ''}
</div>
