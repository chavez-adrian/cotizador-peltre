...
export function generateQuoteHTML(data, options = {}) {
  ...
  const logoPath = join(__dirname, '..', 'data', 'logo_pn.png');
  const logoHtml = existsSync(logoPath)
    ? `<img src="/data/logo_pn.png" alt="pp.peltre" style="max-height:60px;">`
...
/* Header 3 columns */
.header { display: grid; grid-template-columns: 1fr 1fr 1fr; align-items: flex-start; padding-bottom: 12px; border-bottom: 1px solid #ccc; margin-bottom: 14px; gap: 12px; position: relative; }
.header-logo { }
...
.quote-block h2 { font-size: 36px; font-weight: 700; color: #222; letter-spacing: 1px; line-height: 1; margin-bottom: 6px; }
.quote-num { color: #C05000; font-weight: 700; }
.quote-meta { font-size: 11px; color: #444; line-height: 1.9; }
...
/* Payment terms */
.pago-line { font-size: 11px; margin-bottom: 14px; }

/* Products table */
table.products { width: 100%; border-collapse: collapse; margin-bottom: 16px; font-size: 11px; }
table.products thead tr { background: #e8e8e8; }
table.products th { color: #222; font-size: 10.5px; font-weight: 700; padding: 6px 7px; text-align: left; border: 1px solid #ccc; }
table.products th.num { text-align: right; }
table.products td { padding: 5px 7px; border: 1px solid #ccc; vertical-align: middle; }
...
/* Bottom: notes + totals */
.bottom-section { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; gap: 20px; }
.notes-block { flex: 1; }
...
.totals-table tr.total-row td { font-weight: 700; font-size: 13px; border-bottom: none; border-top: 2px solid #222; }
  ...
  <div class="header">
    ...
    <div class="quote-block">
      ...
      <div class="quote-meta">
        Fecha: ${esc(fecha)}<br>
        ${data.id ? `<span class="quote-num">N&ordm; Cotizaci&oacute;n: ${esc(String(data.id))}</span><br>` : ''}
  ...
  <div class="client-section">
    ...
    <div class="client-block">
      ...
      <p>
        ...
        ${(c.celEntrega || c.telefonoEntrega) ? `Telefono: ${esc(c.celEntrega || c.telefonoEntrega)}` : ''}${c.emailEntrega ? ` , Correo: ${esc(c.emailEntrega)}` : ''}<br>
        ${c.nombreCorto ? `Referencia Cliente: ${esc(c.nombreCorto)}` : (c.referencia ? `Referencia Cliente: ${esc(c.referencia)}` : '')}
      </p>
    </div>
  </div>
