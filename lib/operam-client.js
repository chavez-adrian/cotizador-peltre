...
// API, y se corta tambien si una pagina viene incompleta o vacia (guarda contra un
// total mal informado, que dejaria el ciclo girando).
...
async function volcarPaginado(ruta) {
  ...
  for (let skip = 0; skip < total; skip += PAGINA_CATALOGO) {
    ...
    total = Number(data.total) || filas.length;
...
export async function listarItemsCompletos() {
  return volcarPaginado('/api/v3/inventory/items');
}
