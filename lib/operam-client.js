...
// Primer elemento del domicilio de entrega: la calle con su NUMERO INTERIOR (#332).
// `calle` ya trae el numero exterior pegado desde el paso Envio, asi que el interior se
// concatena ahi con la convencion mexicana ("Calle 123 Int. 4, Colonia, ...") en vez de
// ir como elemento suelto entre comas, que se leeria como un dato mas del domicilio.
// Sin interior la cadena queda IDENTICA a la de #328: asi la huella del quote solo se
// mueve para las cotizaciones cuyo cliente si tiene interior capturado.
// "Int." solo antecede a un interior DESNUDO (empieza con digito o mide hasta 3): los
// vendedores capturan "Dept 301", "Local 11" o "Mz. 4 Lts. 13 y 15", que ya se explican
// solos o ni siquiera son un interior, y esos van tal cual despues de la calle.
function calleConInterior(cliente) {
  const calle = String(cliente.calle || '').trim();
  const interior = String(cliente.numInt || '').trim();
...
export function armarContenidoQuote(data) {
  ...
  return {
    ...
    deliveryAddress: [calleConInterior(c), c.colonia, c.cpEntrega, c.municipio, c.estado].filter(Boolean).join(', '),
