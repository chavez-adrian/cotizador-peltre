...
// Va al FINAL del objeto a proposito: asi una huella nueva sin ese campo es
// byte-identica a las que se guardaron antes de #403, que es lo que le permite a
// contenidoQuoteCambio comparar contra ellas sin declarar un cambio inexistente.
...
// La huella SIN la lista del encabezado: la forma exacta que tenian las huellas
// guardadas antes de #403. Comparar por aqui es lo que evita que agregar un campo
// vuelva "cambio" a toda cotizacion ya subida. null si la huella no es la que este
// modulo produce (nunca deberia pasar; se trata como desconocida).
function huellaSinLista(huella) {
  try {
...
// justo lo que #406 decide aparte.
export function contenidoQuoteCambio(data, huellaPrevia, opciones = {}) {
  ...
  const previaSinLista = huellaSinLista(huellaPrevia);
  if (previaSinLista === null || previaSinLista !== huellaPrevia) return true;
  return huellaSinLista(actual) !== previaSinLista;
}
