...
// camina el total con skip (verificado en vivo: skip pagina de verdad, paginas
// sin traslape). Se pagina en vez de pedir un limit generoso porque el pool de
// genericos crece por diseno (#81): un tope fijo truncaria la dedup en silencio,
// que es exactamente el modo de falla que este issue arreglo.
//
...
export async function buscarClientesPorRfc(rfc) {
  ...
  let clientes = [];
  let total = Infinity;
  for (let skip = 0; skip < total; skip += PAGINA_CLIENTES_RFC) {
    let data;
    try {
    ...
    // Pagina incompleta = fin del pool, aunque el total diga otra cosa (guarda
    // contra un total mal informado, que dejaria el ciclo girando).
    if (pagina.length < PAGINA_CLIENTES_RFC) break;
  }
  return clientes;
}
