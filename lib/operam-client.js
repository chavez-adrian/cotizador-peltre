...
//   - las notas y el formato (PDF/HTML, fotos): presentacion. Las notas viajan al quote
//     dentro de comments, asi que un cambio de SOLO notas deja el comments de Operam
//     desactualizado; es una decision explicita de Adrian (texto libre) frente al costo
//     de reescrituras espurias.
...
// Plazo de vigencia en dias (#115), derivado de las dos fechas que ya viajan en data.
// null si falta alguna o no son fechas validas: sin plazo que comparar es mejor no
// inventarlo -- la huella simplemente no lo incluye, como antes de #115.
function diasDeVigencia(d) {
  if (!d.fecha || !d.vigencia) return null;
  const desde = new Date(d.fecha).getTime();
  const hasta = new Date(d.vigencia).getTime();
...
export function huellaContenidoQuote(data) {
  const { items, custRef } = armarContenidoQuote(data || {});
  const d = data || {};
