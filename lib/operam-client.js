...
export async function subirCotizacionOperam(data) {
  ...
  const payload = {
    ...
    payment: 9,
    OrderDate: orderDate,
    // HITL #68 (quote 1160): la API del quote IGNORA valid_until (responde result:true
    // sin error, pero el GET del quote no lo expone). La vigencia real se entrega en
    // comments ("Valido hasta: ..."); el label "Valido hasta" de la UI de Operam parece
    // auto-derivado (ord_date + 30d). Se deja valid_until por si una version futura lo
    // soporta (es inocuo).
