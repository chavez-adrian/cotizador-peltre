...
export function huellaContenidoQuote(data) {
  const { items, custRef, comments, deliverTo, deliveryAddress } = armarContenidoQuote(data || {});
  const d = data || {};
...
export async function subirCotizacionOperam(data, { verificarListaPrecios = true } = {}) {
  ...
  const { items, comments, custRef, orderDate, deliverTo, deliveryAddress } = armarContenidoQuote(data);
