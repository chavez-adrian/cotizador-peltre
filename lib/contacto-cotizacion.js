...
// numero mas corto no identifica a nadie y no produce llave.
function llave(telefono) {
  const diez = ultimos10(telefono);
  return diez.length === 10 ? diez : null;
}

// El celular que se ANOTA al nacer la cotizacion. El telefono del cliente manda
// (es el obligatorio del formulario y el que mueve el embudo en el hook de
// cotizacion); el celular de entrega es el respaldo.
export function celularAlNacer(cliente) {
  const c = cliente || {};
  return llave(c.telefono) || llave(c.celEntrega);
}

// El campo propio ya anotado, o null si la cotizacion es anterior a #342.
export function celularAnotado(cotizacion) {
  return llave(cotizacion && cotizacion.contactoCelular);
}
...
export function celularesDeCruce(cotizacion) {
  ...
  return [llave(cli.telefono), llave(cli.celEntrega)].filter(Boolean);
}
...
function clienteOperamDe(cotizacion) {
  const cli = (cotizacion && cotizacion.data && cotizacion.data.cliente) || {};
  return (cotizacion && cotizacion.clienteOperam) ?? cli.customerId ?? null;
}
...
function celularDeOperam(clienteOperam, telefonosOperam) {
  ...
  for (const casilla of ORDEN_CASILLA) {
    for (const c of candidatos) {
      ...
      const k = llave(c.telefono);
      if (k) return { contacto: k, casilla };
    }
  }
  return null;
}
...
export function contactoDeCotizacion(cotizacion, { contactos, telefonosOperam } = {}) {
  ...
  const cli = (cotizacion && cotizacion.data && cotizacion.data.cliente) || {};
  const tecleado = llave(cli.telefono) || llave(cli.celEntrega);
  if (tecleado) {
    if (conocidos.has(tecleado)) return { contacto: tecleado, fuente: FUENTES.CRUCE, crear: false };
    return { contacto: tecleado, fuente: FUENTES.TELEFONO, crear: true, clienteOperam };
  }
