...
// vCard del prospecto (issue #165): nombre, celular, correo y empresa -- solo
// eso, para que "Agregar contacto" prellene lo minimo util. Sin servicios
// externos: es un string armado a mano, CRLF por el RFC. Los campos opcionales
// se omiten si estan vacios (una EMAIL: vacia rompe algunos lectores de vCard).
export function vcardDeProspecto(prospecto) {
  const p = prospecto || {};
  const lineas = ['BEGIN:VCARD', 'VERSION:3.0', `FN:${escapeVCard(p.nombre)}`];
  if (limpio(p.celular)) lineas.push(`TEL;TYPE=CELL:${limpio(p.celular)}`);
