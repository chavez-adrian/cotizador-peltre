...
// Zona de la fabrica. La fecha de la nota tiene que leerse en hora de CDMX: el
// instante que inyecta el endpoint es UTC, y en UTC una captura despues de las
// 6pm locales se fechaba AL DIA SIGUIENTE. La tarjeta del pipeline la muestra en
// local (prospectos-logica.js), asi que el contacto del telefono terminaba
// diciendo otra cosa que el tablero sobre la MISMA captura.
//
// Se nombra la zona IANA en vez de restar seis horas fijas: el desfase es una
// regla politica, no aritmetica (CDMX dejo el horario de verano en 2022 y nada
// garantiza que no vuelva; restar 6h codificaria esa decision en el codigo). Y
// sigue siendo determinista: toLocaleDateString solo varia por maquina cuando se
// dejan locale y timeZone implicitos -- fijando los dos, la salida la decide ICU
// y no el entorno, aqui y en Render igual. 'en-CA' es el atajo estandar a
// YYYY-MM-DD, no una preferencia de idioma.
const ZONA_FABRICA = 'America/Mexico_City';

function fechaEnZonaFabrica(iso) {
  const instante = new Date(iso);
  if (Number.isNaN(instante.getTime())) return '';
  try {
    return new Intl.DateTimeFormat('en-CA', {
      timeZone: ZONA_FABRICA, year: 'numeric', month: '2-digit', day: '2-digit',
    }).format(instante);
  } catch {
    // Un Node compilado con small-icu no conoce la zona y tira RangeError. Sin
    // este respaldo la excepcion subiria por vcardDeProspecto hasta reventar el
    // armado del mensaje, y el vendedor se quedaria SIN ALERTA -- no solo sin la
    // fecha en el huso correcto. Se cae al corte UTC, que es lo que habia antes.
...
// La nota abre con la fecha, que declara que esto es una FOTO DEL MOMENTO: la
// fuente de verdad sigue siendo la tarjeta del pipeline, nunca el contacto del
// telefono. La fecha va SOLA, sin rotulo de procedencia: el vendedor solo recibe
// estas fichas por la alerta de mayoreo, asi que anunciarlo en cada contacto
// gasta el primer renglon de lo que se ve en la pantalla del telefono.
//
...
function lineaNote(p) {
  ...
  // El nucleo NUNCA consulta el reloj: el instante lo inyecta el endpoint, para
  // que esto sea determinista en tests. Aqui solo se traduce al huso de la
  // fabrica. Un prospecto sin fecha simplemente no lleva encabezado -- la nota
  // vale por sus lineas de contexto, no por el renglon vacio que las precede.
  const fecha = fechaEnZonaFabrica(limpio(p.fechaCaptura));
  const renglones = fecha ? [fecha, ...filas] : filas;
  return `NOTE:${escapeVCard(renglones.join('\n'))}`;
}
