...
// #342: la liga Oportunidad -> Contacto la decide un solo nucleo (ADR-0016).
import { celularesDeCruce, esContactoSinCaptura } from './contacto-cotizacion.js';
import { COLUMNAS_PIPELINE, COLUMNA_LABELS, esSalida, etiquetaFolioOperam } from '../public/js/pipeline-logica.js';
// #322: los RFC genericos y su normalizacion son los del alta y la dedup, nunca
// una lista local: un RFC capturado con espacios o en minusculas es el mismo.
...
// #400: la etiqueta "Ya tiene Cliente Operam, falta cotizar" (CONTEXT.md) leida
// literal -- las DOS mitades, no solo la liga: vale mientras el Contacto tiene
// Cliente Operam y TODAVIA no cotiza, que es la fuga que la cola vigila. La
// mitad de "ya cotizo" es la MISMA de la escalera (tieneCotizacion), asi que un
// Contacto que el cotizador dio de alta AL cotizar deja de llevarla. Este es EL
// predicado: lo comparten la tarjeta de la lista de Prospectos y la cola Hoy,
// que solo pintan lo que el servidor ya juzgo con su visibilidad.
export function faltaCotizar(p, cotizaciones = []) {
  return clienteIdDe(p) != null && !tieneCotizacion(p, cotizaciones);
}
...
export function filaTabla(p, cotizaciones = [], ahora = new Date()) {
  ...
  return {
    ...
    gafete: gafeteDe(p),
    // #342: el Contacto que nacio de la migracion, no de una captura. La ficha
    // lo dice ("venido de Operam") para que no parezca prospectado.
    sinCaptura: esContactoSinCaptura(p),
...
// Orden de salida fijo (CONTEXT.md "Que sigue / Que falta"). Las dos primeras
// son del prospecto (#321) y las dos ultimas del cliente ligado (#322).
...
export function queFalta(p, cotizaciones = []) {
  ...
  const referencia = cotizacionReferencia(clienteIdDe(p), cotizaciones);
  if (referencia) {
    const cli = (referencia.data && referencia.data.cliente) || {};
    const rfc = normalizarRfc(cli.rfc);
    if (!rfc || RFC_GENERICOS.has(rfc)) llaves.push('datos_fiscales');
    // La MISMA condicion con la que `buildBranchGenerico` (lib/alta-generica.js)
    // omite el domicilio del branch: sin calle Y CP el alta no registro ninguno,
    // y por eso se leen los MISMOS dos campos -- el CP del domicilio de entrega
    // se llama `cpEntrega` en lo que guarda la cotizacion, nunca `cp`.
    const calle = String(cli.calle || '').trim();
    const cp = String(cli.cpEntrega || '').trim();
    if (!calle || !cp) llaves.push('domicilio');
  }
  return llaves;
}

// #322: la cotizacion de REFERENCIA del cliente ligado -- la unica evidencia de
// con que datos nacio el cliente en Operam (#81/#83), porque la tabla NUNCA
// consulta Operam (CONTEXT.md "Que sigue / Que falta"). Es la mas reciente de
// las que se subieron A ESE cliente (`data.cliente.customerId`, comparado como
// texto porque llega numero o cadena segun el camino); si ninguna lo nombra, la
// mas reciente de las ligadas. Sin ninguna cotizacion ligada no hay evidencia:
// devuelve null y la fila NO inventa huecos.
function cotizacionReferencia(clienteId, cotizaciones) {
  if (clienteId == null) return null;
  const porFecha = (cotizaciones || []).slice()
    .sort((a, b) => new Date(b.fecha || 0) - new Date(a.fecha || 0));
  const delCliente = porFecha.find(c =>
    String(((c.data && c.data.cliente) || {}).customerId ?? '') === String(clienteId));
  return delCliente || porFecha[0] || null;
}
...
// guarda la tarjeta del prospecto, y el celular con el que la cotizacion cruza,
// que rescata las cotizaciones nacidas sin pasar por la tarjeta. Sin
// duplicados: una misma cotizacion puede cumplir las dos.
//
// #342: ese celular lo decide `celularesDeCruce` -- el campo propio de la
// cotizacion (la liga fija de ADR-0016) y, solo mientras no lo tenga, lo
// tecleado. Por eso corregir el telefono de una cotizacion ya ligada no la
// mueve a la tarjeta de otra persona.
export function cotizacionesDelProspecto(p, cotizaciones = []) {
  ...
  const ligadas = (cotizaciones || []).filter(c => {
    ...
    return celularesDeCruce(c).includes(llave);
  });
