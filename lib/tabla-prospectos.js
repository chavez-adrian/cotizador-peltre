...
import { COLUMNAS_PIPELINE, esSalida, etiquetaFolioOperam } from '../public/js/pipeline-logica.js';
...
// (CONTEXT.md "Que sigue / Que falta"). El escalon 'cliente' lo llena #320:
// mientras tanto sale con accion vacia, nunca roto.
export function queSigue(p, cotizaciones = [], ahora = new Date()) {
  ...
  if (estado === 'cotizado') {
    ...
    const mas = masCotizaciones > 0 ? ` y ${masCotizaciones} más` : '';
    return {
      tipo: 'seguimiento',
      accion: `Seguimiento a la ${folio}, ${TEXTO_PASO[paso] || 'enviada hoy'}${mas}`,
      cotizacionId: c.id,
      folio,
      paso,
      dias,
