...
// Todas las descripciones del catalogo que aparecen en el texto, en el ORDEN en que
// el SAT las imprime (issue #390: la constancia puede traer varios regimenes y el
// panel preselecciona el primero). Cada aparicion reclama su tramo del texto y una
// descripcion mas corta que caiga dentro de un tramo ya reclamado no cuenta: el
// orden por longitud sigue decidiendo quien gana ese tramo.
function mapearRegimenesPorTexto(texto) {
  const t = norm(texto);
  const tramos = [];
  for (const [codigo, descNorm] of REGIMENES_POR_TEXTO) {
    for (let pos = t.indexOf(descNorm); pos !== -1; pos = t.indexOf(descNorm, pos + 1)) {
      const fin = pos + descNorm.length;
      if (tramos.some(r => pos < r.fin && fin > r.pos)) continue;
      tramos.push({ pos, fin, codigo });
    }
  }
  return [...new Set(tramos.sort((a, b) => a.pos - b.pos).map(r => r.codigo))];
}
...
export function parsearCSF(textoCrudo) {
  ...
  // El codigo numerico (formato viejo) sigue teniendo prioridad sobre la
  // descripcion. Con varios regimenes, el elegido es el primero de la lista.
  const regimenesFiscales = (() => {
    const codigos = [...texto.matchAll(/R[eé]gimen\s*Fiscal\s*:[ \t]*(\d{3})/gi)].map(m => m[1]);
    if (codigos.length) return [...new Set(codigos)];
    return mapearRegimenesPorTexto(texto);
  })();
  const regimenFiscal = regimenesFiscales[0] || '';
