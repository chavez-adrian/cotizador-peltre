...
// [codigo, descripcion legible (para mostrar), descripcion normalizada (para comparar)]
...
].map(([codigo, desc]) => [codigo, desc, norm(desc)]).sort((a, b) => b[2].length - a[2].length);

function mapearRegimenPorTexto(texto) {
  const t = norm(texto);
  for (const [codigo, , descNorm] of CATALOGO_REGIMENES) {
    if (t.includes(descNorm)) return codigo;
  }
  return '';
}

function labelRegimen(codigo) {
  const fila = CATALOGO_REGIMENES.find(([c]) => c === codigo);
  return fila ? fila[1] : '';
}
