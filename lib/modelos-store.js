...
// Las 32 columnas del bloque en el orden del archivo: es el que ve quien edita
// la semilla y el que devuelve listar(), venga de la base o del fallback.
const COLUMNAS = [
...
// El tipo sale del que trae el archivo: NUMERIC lo que se mide, TEXT lo que se
// lee. Solo se declaran las numericas y el resto se deriva, para que agregar una
// columna al bloque no pueda dejar dos listas fuera de sincronia.
...
const COLUMNAS_TEXTO = COLUMNAS.filter(c => c !== 'modelo' && !COLUMNAS_NUMERO.includes(c));
...
// el orden del archivo los valores caerian en la columna equivocada. El NULLIF
// es la guarda del cast: una medida que quede vacia en el archivo es la columna
// sin dato, no un ''::numeric que tumbaria la siembra completa.
const SEMILLA = `
  INSERT INTO modelos (${['modelo', ...COLUMNAS_TEXTO, ...COLUMNAS_NUMERO].join(', ')})
  SELECT ${['modelo', ...COLUMNAS_TEXTO].map(c => `m->>'${c}'`).join(', ')},
         ${COLUMNAS_NUMERO.map(c => `NULLIF(m->>'${c}', '')::numeric`).join(', ')}
