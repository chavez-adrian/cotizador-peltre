...
import { esCodigoCalca } from '../public/js/calcas-logica.js';
...
export function calcularPaquetes(items, totalConIVA = 0) {
  ...
  for (const item of items) {
    ...
    // La calca (issue #91) va APLICADA sobre la pieza: no ocupa caja ni pesa
    // aparte. Sin excluirla, slice(0,4) daba 'CAL1' -- ausente de boxMap -- y
    // cada cotizacion con calca arrastraba un warning falso de empaque.
    if (esCodigoCalca(item.codigo)) continue;
