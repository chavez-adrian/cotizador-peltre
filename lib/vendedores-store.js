...
// Columna agregada por #163 (alerta por correo de captura publica de mayoreo): el
// registro no traia correo -- destinatariosAlertaMayoreo (lib/alerta-mayoreo.js)
// lo necesita para avisar a quien tiene el permiso de asignacion. Mismo patron
// ALTER TABLE IF NOT EXISTS que las dos columnas anteriores.
const ALTER_EMAIL = `
  ALTER TABLE vendedores ADD COLUMN IF NOT EXISTS email TEXT
`;

// Columna agregada por #280 (permiso de precio de calca por vendedor), mismo
// motivo y patron que ALTER_PUEDE_FIJAR_LISTA.
const ALTER_PUEDE_PRECIO_CALCA = `
  ALTER TABLE vendedores ADD COLUMN IF NOT EXISTS puede_precio_calca BOOLEAN
`;

// Columna agregada por #296 (matriz de listas habilitadas, ADR-0015), mismo
// motivo y patron que las anteriores. Es JSONB porque la celda guarda una
// COLECCION de ids de lista de Operam, y el NULL es informacion: distingue "sin
// configurar" (migra desde puede_fijar_lista) de "ninguna" (arreglo vacio).
const ALTER_LISTAS_HABILITADAS = `
  ALTER TABLE vendedores ADD COLUMN IF NOT EXISTS listas_habilitadas JSONB
`;

const INSERT = `
  INSERT INTO vendedores (id, name, pin, role, operam_id, tope_descuento, puede_fijar_lista, puede_asignar, email, puede_precio_calca, listas_habilitadas)
  SELECT (v->>'id')::int, v->>'name', v->>'pin', v->>'role',
         (v->>'operam_id')::int, (v->>'topeDescuento')::numeric,
         (v->>'puedeFijarLista')::boolean, (v->>'puedeAsignar')::boolean, v->>'email',
         (v->>'puedePrecioCalca')::boolean, v->'listasHabilitadas'
...
async function ensureSchema() {
  if (!schemaListo) {
    schemaListo = query(SCHEMA)
      ...
      .then(r => (r === null ? null : query(ALTER_EMAIL)))
      .then(r => (r === null ? null : query(ALTER_PUEDE_PRECIO_CALCA)))
      .then(r => (r === null ? null : query(ALTER_LISTAS_HABILITADAS)))
...
function filaAEntrada(row) {
  ...
  // Correo (#163): igual que los anteriores, ausente no se ve distinto de un
  // vendedor del JSON que nunca tuvo el campo.
  if (row.email) v.email = row.email;
  // Permiso de precio de calca (#280): mismo criterio que puede_fijar_lista.
  if (row.puede_precio_calca) v.puedePrecioCalca = true;
  // Listas habilitadas (#296): a diferencia de los permisos de arriba, el
  // arreglo VACIO se conserva -- "ninguna lista" es una decision del admin y
  // omitirlo lo volveria "sin configurar", que la migracion de lectura leeria
  // como los escalones de volumen. Solo el NULL (columna nunca escrita) se omite.
  if (row.listas_habilitadas != null) v.listasHabilitadas = row.listas_habilitadas;
  return v;
}
