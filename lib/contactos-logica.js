...
export function planificarContactos({
  ...
  // la solicitud de borrado se atiende a mano). Acepta cadenas (celular10) o
  // filas del store ({ celular10, motivo, excluidoEn }). Se respeta cerrando la
  // puerta de entrada (anotar) y la de salida (inactivar) del mismo mapa: un
  // celular de aqui no entra nunca a `deseadas`, asi que no puede crearse,
  // actualizarse, adoptarse ni reactivarse -- y se salta explicitamente en el
  // barrido de inactivacion para que tampoco cuente como candidato ni reciba una
  // escritura que ya no le corresponde (el borrado real lo hizo el
  // procedimiento manual, esta fila -- si aun existe -- no necesita tocarse).
  const excluidosSet = new Set(
    (excluidos || []).map(e => (typeof e === 'string' ? e : e?.celular10)).filter(Boolean)
  );
