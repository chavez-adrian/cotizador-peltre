...
//
// Los prospectos no necesitan la misma regla: prospectosStore.listar() propaga
// su fallo y tumba la pasada entera antes de llegar aqui.
...
export function planificarContactos({ prospectos = [], clientes = [], mapeo = [], libreta = [] } = {}) {
  ...
  for (const ficha of mapeo || []) {
    ...
    // Un ADOPTADO no llega aqui nunca, y es la mitad de este ticket: no era
    // nuestro. Que su origen desaparezca del sistema no autoriza a etiquetar el
    // contacto de alguien mas (ADR-0013). Su prueba esta escrita como candado.
