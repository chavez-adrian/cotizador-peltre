...
//
// La parte de PERSONA se normaliza a Titulo con EL titulador del repo (#247,
// unificado en #293): Operam entrega el nombre del SAT/contacto en mayusculas y
// "BRENDA GARCIA" grita en una lista de chats. La guarda de lo que ya viene
// escrito en mixto es del titulador -- aqui no se repite. Va sin
// { siglasCortas: true } a proposito: medido en la primera carga real, los
// tokens cortos del campo de persona eran nombres de pila y abreviaturas
// ("DAN", "MA."), no siglas comerciales.
//
...
export function nombreVisible({ persona, empresa, ciudad } = {}) {
  const quien = aTitulo(limpio(persona));
...
// Ficha de un CONTACTO del cotizador (#227; etiqueta desde #344/ADR-0016). La
// marca de origen dice de que fuente salio, y desde #342 esa fuente tiene dos
// clases: el Contacto que alguien capturo por alguna puerta -- ese es el
// prospecto -- y el que nacio de la migracion para que una Oportunidad
// historica tuviera de quien ser, que nadie prospecto nunca. Los dos merecen
// ficha (a los dos se les escribe por WhatsApp) y los dos ocupan el mismo
// escalon de la precedencia de ADR-0013; lo unico que cambia es no llamarle
// prospecto a quien no lo es.
function fichaDeProspecto(prospecto) {
  const data = prospecto.data || {};
  const empresa = limpio(data.empresa);
  const clase = esProspecto(prospecto) ? 'prospecto' : 'contacto';
  return {
    ...
    origen: `cotizador:${clase}:${prospecto.id}`,
  };
}
...
function fichaDeCliente(entrada) {
  ...
  const empresa = nombreCorto || aTitulo(limpio(entrada.razonSocial));
...
// Ficha de un comprador de la tienda en linea (#255, ADR-0014). La entrada es
// UNA fila de la tabla pedidos_shopify: el telefono llega YA resuelto (la regla
// del pais corre al ingerir, en lib/pedidos-shopify-logica.js) y el nombre es el
// de la direccion de la que salio ese telefono -- quien recibe el paquete puede
// no ser quien pago.
//
// El numero de pedido ocupa el lugar de la empresa en el nombre visible, que es
// justo lo que quien contesta el WhatsApp necesita leer primero: "de que pedido
// me habla". La organizacion va VACIA a proposito: un comprador de menudeo no
// tiene empresa, y meter ahi el pedido lo duplicaria en un campo que WhatsApp
// ni siquiera muestra.
function fichaDeClienteEnLinea(fila) {
  return {
    celular10: ultimos10(fila.telefono),
    nombreVisible: nombreVisible({ persona: fila.nombre, empresa: limpio(fila.pedido) }),
    telefono: aFormatoWhatsApp(fila.telefono),
    correo: limpio(fila.correo),
    organizacion: '',
    origen: `cotizador:enlinea:${limpio(fila.pedido)}`,
  };
}

// "Mi ultimo pedido" es el caso normal de quien escribe: si un telefono aparece
// en varios pedidos, la ficha lleva el del createdAt MAYOR. Se decide aqui y no
// en la consulta (#254) para que la regla se pueda probar sin montar la tabla.
function ultimoPedidoPorTelefono(pedidos) {
  const porCelular = new Map();
  for (const fila of pedidos || []) {
    const llave = ultimos10(fila.telefono);
    const previo = porCelular.get(llave);
    if (!previo || String(fila.creadoEn || '') > String(previo.creadoEn || '')) porCelular.set(llave, fila);
  }
  return [...porCelular.values()];
}
...
//
// La fuente de PEDIDOS (#255) entra por la misma puerta y por la misma razon:
// el sondeo de Shopify escribe a una tabla y el plan la lee, asi que una tabla
// vacia es indistinguible de un token caducado o de un sondeo que nunca corrio.
// Y como los clientes en linea NUNCA pierden respaldo por el paso del tiempo
// (ADR-0014, "Para siempre"), que su fuente se vacie no puede significar nunca
// una baja legitima: significa que la fuente no esta.
function sinEvidenciaDeDesaparicion(clientes, pedidos) {
  ...
  if ((pedidos || []).length === 0) {
    return {
      categoria: 'datos',
      motivo: 'fuente de pedidos vacia: la tabla de pedidos de la tienda en linea no devolvio ninguna fila en esta pasada, y eso es indistinguible de un sondeo caido. No se inactivo ninguna ficha: sin la fuente completa no hay evidencia de que un origen haya desaparecido.',
    };
  }
  return null;
}
...
//
// Tres fuentes: los compradores de la tienda en linea (#255), los prospectos
// del cotizador (#227) y los telefonos de los clientes de Operam (#228). Cuando
// comparten celular gana el ultimo anotado, y por eso el orden en que se anotan
// ES la precedencia de ADR-0013/ADR-0014: cliente de Operam > prospecto >
// cliente en linea.
//
...
export function planificarContactos({
  prospectos = [], clientes = [], pedidos = [], mapeo = [], libreta = [], excluidos = [],
} = {}) {
  // Lista de exclusion por celular (#259, spec #254; ADR-0013 "Consecuencias":
  // la solicitud de borrado se atiende a mano). Filas del store
  // (contactos-excluidos-store.js: { celular10, motivo, excluidoEn }), misma
  // forma que `mapeo`. Se respeta cerrando la puerta de entrada (anotar) y la
  // de salida (inactivar) del mismo mapa: un celular de aqui no entra nunca a
  // `deseadas`, asi que no puede crearse, actualizarse, adoptarse ni
  // reactivarse -- y se salta explicitamente en el barrido de inactivacion para
  // que tampoco cuente como candidato ni reciba una escritura que ya no le
  // corresponde (el borrado real lo hizo el procedimiento manual, esta fila --
  // si aun existe -- no necesita tocarse).
  const excluidosSet = new Set((excluidos || []).map(e => e?.celular10).filter(Boolean));
  ...
  const anotar = (ficha) => {
    ...
    if (excluidosSet.has(ficha.celular10)) return;
    deseadas.set(ficha.celular10, ficha);
  };
  // Los compradores de la tienda van PRIMEROS: al fondo de la precedencia
  // (ADR-0014). Un pedido nuevo de un telefono que ya es prospecto o cliente no
  // cambia nada, y esa es la forma mas segura de que entre una fuente nueva.
  for (const fila of ultimoPedidoPorTelefono(pedidos)) anotar(fichaDeClienteEnLinea(fila));
  ...
  for (const ficha of mapeo || []) {
    ...
    // Excluido (#259): el procedimiento manual ya la borro de Google; esta fila
    // -- si aun existe -- no se toca y no cuenta como candidato a inactivacion.
    if (excluidosSet.has(ficha.celular10)) continue;
  ...
  const freno = inactivar.length > 0 &&
    (sinEvidenciaDeDesaparicion(clientes, pedidos) || frenoDeInactivacion(inactivar.length, propiasActivas));
  if (freno) {
    errores.push(freno);
    inactivar.length = 0;
  }
