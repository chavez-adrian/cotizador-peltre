...
// El maximo de las DOS tablas. Va en consulta aparte y tolerando el fallo de la
// de Contactos: si `prospectos` todavia no existe (base nueva en la que nadie
// ha leido prospectos), un INSERT que la mencionara en un subquery reventaria en
// el PARSEO, no en la ejecucion -- ninguna guarda dentro del SQL lo evita.
async function siguienteId() {
  let maxContacto = 0;
  try {
    const r = await query('SELECT COALESCE(MAX(id), 0) AS m FROM prospectos');
    if (r) maxContacto = Number(r.rows[0].m);
  } catch { /* la tabla de Contactos aun no existe: solo cuenta la de aqui */ }
  const r = await query('SELECT COALESCE(MAX(id), 0) AS m FROM oportunidades');
  return Math.max(maxContacto, r ? Number(r.rows[0].m) : 0) + 1;
}
...
export async function crear(entry) {
  ...
  const r = await query(
    `INSERT INTO oportunidades (id, fecha, contacto_id, contacto10, vendedor, etapa, eventos, data)
     VALUES ($8, $1, $2, $3, $4, $5, $6::jsonb, $7::jsonb)
     RETURNING id`,
