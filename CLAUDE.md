...
- **Numeracion de la cotizacion (ADR-0009, decidido 2026-07-28 — IMPLEMENTACION PENDIENTE en #110+#111):** el numero de la cotizacion **es el folio de Operam**, no el id interno del registro. Para poder imprimirlo hay que **invertir el orden actual**: hoy se genera el documento y DESPUES se sube a Operam de forma asincrona (#83), asi que cuando el PDF se descargo el folio todavia no existe. Fallback obligatorio: si la subida falla el documento se entrega igual, sin numero y como pre-cotizacion; el bueno se re-comparte desde el historial (los GET regeneran desde `data`). Las PRE no muestran numero. **Estado ACTUAL del codigo (el bug que #110 arregla): el PDF no imprime NINGUN numero** — `generateQuotePDF` sabe pintar `No. Cotizacion: N` pero ni el POST ni el GET le pasan el id; los dos caminos del HTML si lo hacen, y por eso el HTML muestra el id interno. #110 y #111 se liberan JUNTOS: por separado los clientes reciben dos numeraciones distintas.
