...
| `oportunidades.js` | Nucleo PURO de las tarjetas del tablero (#340, ADR-0016): `tarjetasOportunidades(prospectos, cotizaciones)` -> una tarjeta por Oportunidad, sin la del prospecto que ya cotizo (liga por `cotizacionesDelProspecto`); lo sirve `GET /api/oportunidades`, el UNICO endpoint del tablero |
...
| `tabla-prospectos.js` | Nucleo PURO de la Tabla de prospectos (#306, tickets #313-#323): `estadoProspecto` y `filaTabla` traducen el prospecto a la fila que pinta `/prospectos` (`/leads` redirige ahi); el navegador NO calcula derivados, los pide a `GET /api/prospectos/tabla` |
