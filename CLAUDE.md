...
| `alta-cliente.js` | EL modulo **Alta de cliente** (#364, ADR-0017): `darDeAlta(solicitud, deps = {})` recorre la secuencia unica del alta -- dedup en capas, crear o reutilizar, dimensiones, domicilio de entrega (POST solo si no hay uno equivalente; PUT **solo sobre el recien creado**, con una bandera que decide el modulo), relectura y comparacion SIEMPRE, Cel -- y devuelve VALORES: alta lograda, pregunta al vendedor (candidatos con sus tres salidas, o el celular ligado a otra razon social) o bloqueo con motivo. Nunca un status HTTP, y no importa cotizaciones ni Express: los handlers solo traducen. Cada paso lleva `mensaje` del glosario y `detalle` tecnico (**Mensaje en dos capas**), y el que no aplica sale `omitido` con su motivo. Operam, el padron, la auditoria (`clientes-log.js`), el vendedor y la liga del Contacto entran como `deps` con fallback a las funciones reales; el adaptador en memoria de los tests implementa las mismas (`test/helpers/operam-memoria.js`, cero `fetch`). Devuelve el `clienteId` tambien en el bloqueo: quien pide el alta lo persiste antes de responder y por eso un reintento no crea un segundo cliente |
