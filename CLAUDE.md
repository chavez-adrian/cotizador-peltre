...
| `contactos-logica.js` / `contactos-io.js` / `google-contactos.js` / `contactos-store.js` | Sync de contactos a la libreta de Google para que WhatsApp Business muestre nombres (#224/#227/#228, ADR-0013): nucleo puro que devuelve el plan `{crear, actualizar, inactivar}` con **mascara explicita por entrada** desde DOS fuentes — prospectos y telefonos de clientes de Operam, que se anotan despues y por eso GANAN (precedencia de ADR-0013) + motor que lo ejecuta SECUENCIAL + cliente REST de People API (calcado de `dropbox.js`, cero dependencias) + mapeo `celular10 -> resourceName/etag/clase/huella` en Neon. `aFormatoWhatsApp` es EL punto unico del formato del telefono (#226) |
