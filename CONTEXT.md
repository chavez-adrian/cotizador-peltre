...
La variante de captura en la que el prospecto se registra a sí mismo desde el formulario web de mayoreo (canal Formulario web); nace en No Asignado, sin vendedor. Pide nombre y apellido por separado (se conservan como un solo nombre — dos campos obligatorios inducen datos más completos), celular con código de país (la llave de identidad), código postal del que el sistema deriva la ciudad (con captura manual de ciudad como respaldo si el CP no resuelve), correo obligatorio, tipo de proyecto (pre-asigna el segmento de Operam; la opción elegida se conserva textual en las notas porque varias opciones caen en un mismo segmento) y cantidad estimada. La cantidad omite deliberadamente +6,000: ese nivel exige negociación humana y no debe entrar por autoservicio (decisión 2026-08-15). "Otro" como tipo de proyecto exige especificar cuál. Opcionales: empresa o proyecto, cargo, sitio web o redes sociales. La respuesta pública nunca revela si el celular ya es prospecto o cliente — hacia afuera siempre es "gracias, te contactamos"; la regla de Visibilidad aplica solo dentro del equipo. Cada captura pública avisa por correo a quienes tienen el permiso de asignación.
