...
Proceso de verificar que un cliente nuevo no exista ya en Operam antes de crearlo. Para clientes con RFC real: búsqueda exacta por RFC. Para clientes con RFC genérico (`XAXX010101000` o `XEXX010101000`): búsqueda por nombre normalizado (sin acentos, sin artículos, sin preposiciones, sin sufijos corporativos) contra CustName y cust_ref. **Coincidencia de nombre = al menos dos tokens compartidos** (uno solo si el nombre capturado tiene un único token útil); el teléfono es señal independiente y marca candidato por sí solo. Si hay coincidencia, el sistema muestra los candidatos existentes y el vendedor elige uno — o declara que **ninguno es el mismo cliente**, y entonces sí se crea el cliente nuevo, dejando registro del forzado en el log de auditoría. Ésas son las **dos únicas salidas**: no se puede dejar la cotización en PRE y seguir. Mientras el duplicado siga sin resolver la cotización queda con `motivoPre` `'dedup'` y su documento bajo candado (los GET que regeneran responden un aviso), y a las 24 horas se borra. Si el vendedor elige un cliente existente, el sistema muestra sus domicilios de entrega para que seleccione uno existente o cree uno nuevo. El umbral y el escape son de #204; hasta entonces bastaba un token y no había salida (ADR-0001 y su nota del 2026-08-19).
