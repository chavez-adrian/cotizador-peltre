...
- **#62 CERRADO** ✅ — sync Operam post-venta (HITL). Núcleo puro (AC3) + **capa de IO + wiring (AC2)**: remapeo de `hechosDesdeOperam` al mapeo REAL (peltre-operam.md §12: 10=factura/pago, 13=remisión, 30=pedido — NO las etiquetas del MCP); lecturas `listarTransacciones`/`listarPedidos` (operam-client); motor `reconciliarOportunidad` (`lib/sync-operam-io.js`); webhook `POST /api/webhooks/operam` (header secreto + log idempotente Neon) + reconciliación on-demand `POST /api/sync-operam`. AC1 documentado, AC4 N/A (4 etapas AUTO). **Fix del orquestador:** la señal de pago se deriva de `allocated` vs `total` con tolerancia 1% (el `outstanding` del listado de Operam no es fiable — sale ≠ 0 en facturas pagadas; verificado en vivo con El Péndulo). Verificación read-only contra Operam real OK. Mergeado a main. Suite 745/745. **ACTIVADO (2026-06-17):** `OPERAM_WEBHOOK_SECRET` en Render + los 3 webhooks dados de alta en Operam (`Payment/ADD`, `Order/ADD`, `CustDelivery/ADD` → endpoint con header secreto); smoke test 401/200 OK. **Regla de binding (Adrián confirmó):** el número de cotización NUNCA es igual al de pedido → el folio (`folioOperam`) no se usa como `order_`; binding preciso solo vía `data.orderOperam` (fix `a12ea3b`), si no, agregado por cliente. **Mejora propuesta (issue nuevo):** persistir TODOS los folios de la cadena (cotización, pedido, factura nº+ref, remisiones, pagos, notas de crédito) asociados al `order_` — ver "Siguiente".
...
## Siguiente
**PRD #52 COMPLETO: #53–#66 cerrados (14 de 14).** Embudo completo + cotizar stepper (#60) + reunión sobre prospectos y cotizaciones (#65) + decorados con gate y Dropbox (#61) + **sync Operam post-venta (#62)**. El tablero tiene 4 acciones de tarjeta (asignar #57, mover a Seguimiento #56, salidas #59, decorado/calca #61), todas con refId + tests de forma prefijada. **Deuda de proceso: las acciones de tarjeta / cambios visuales idealmente se verifican en navegador (origen del bug de #57); el orquestador puede capturar via chrome-devtools MCP + npm start local.**

**#62 ACTIVADO (2026-06-17):** `OPERAM_WEBHOOK_SECRET` seteado en Render y los 3 webhooks dados de alta en Operam (`Payment/ADD`, `Order/ADD`, `CustDelivery/ADD` → `https://cotizador-peltre.onrender.com/api/webhooks/operam`, header `X-Operam-Webhook-Secret`); smoke test 401 sin secret / 200 con secret. La reconciliación on-demand (`POST /api/sync-operam`) es la red de seguridad. Pendiente operativo del dueño: capturar el payload del primer evento real (revisar `operam_webhooks_log` en Neon) para endurecer el extractor si hiciera falta, y (opcional) subir Render a Starter para no perder webhooks por cold-start.

**Regla de dominio confirmada (Adrián):** el número de cotización NUNCA es igual al número de pedido en Operam → el folio (`folioOperam`) no se usa como `order_` (fix `a12ea3b`: binding preciso solo vía `data.orderOperam`).
...
### Estado de cada AC
- AC1 (marcar decorada + checklist 6 pasos con progreso): VERDE. Dominio `decorados-logica.js` (PASOS_DECORADO, checklistInicial, progresoDecorado; DC1/DC2/DC7/DC8). Ruta `PATCH /api/cotizacion/:id/decorado` activa el checklist 0/6 (DEC1/DEC2/DEC3). Frontend `buildDecoradoControlHtml` pinta "Calca N/6" + los 6 pasos en la tarjeta (Q47/Q49/Q52).
- AC2 (marcar/revertir cada paso): VERDE. Dominio marcarPaso/revertirPaso PUROS (DC3/DC4/DC5/DC6). Ruta `PATCH /api/cotizacion/:id/calca-paso` (DEC4/DEC5/DEC6). Frontend toggle por paso (toggleCalcaPaso).
- AC3 (gate server-side a Pedido liberado): VERDE. Dominio `puedeLiberar` (no decorada=true; decorada parcial=false; decorada completa=true; DC10/DC11/DC12/DC13). Ruta `POST /api/cotizacion/:id/liberar`: decorada+incompleto -> 409 (DEC8), decorada+completo -> 200 (DEC9), no decorada -> 200 (DEC10). Es el PUNTO DE ENFORCEMENT MINIMO que #62 reusara.
- AC4 (paso 6 sube a Dropbox): VERDE. `subirCalcaDropbox` (server.js) FIRE-AND-FORGET via `import('./lib/dropbox.js').upload(path, base64, 'add')`; un fallo o la ausencia de Dropbox (local) NO bloquea ni impide marcar el paso (DEC7). Frontend: input file -> base64 -> ruta (subirCalcaArchivos). Ruta Dropbox: `/PELTRE NACIONAL/CALCAS/POSICIONES/Cot <id> - <nombre>`.
- AC5 (modulo de checklist con pruebas): VERDE. `public/js/__tests__/decorados-logica.test.cjs` (DC1-DC13): completar/revertir cada paso, progreso, puedeLiberar con checklist parcial/completo/no-decorada.

### Diseno (lo que no perder)
- **Dominio puro nuevo `public/js/decorados-logica.js`** (browser-safe, mismo patron que prospectos-logica/pipeline-logica): lo importan server.js (gate + rutas) y pipeline-logica.js (presentacion) y el test .cjs. Exporta PASOS_DECORADO (6 pasos clave+label EN ORDEN), checklistInicial, marcarPaso/revertirPaso (puros, no mutan; normalizan a la forma canonica de 6), progresoDecorado ({completos,total}, tolera ausente=0/6), esDecorada (lee data.decorado o flag al tope), puedeLiberar (acepta cotizacion/oportunidad o checklist crudo).
- **El gate vive en el dominio** (puedeLiberar); la ruta solo lo aplica. **#62 (sync Operam post-venta, AUN NO EXISTE) DEBE pasar por este mismo gate** antes de mover una oportunidad decorada a pedido_liberado — documentado en comentario sobre la ruta `/liberar`. NO se modelo el mapeo estado->etapa post-venta de #62 (fuera de alcance): el "avance" se representa minimamente con `data.pedidoLiberado=true`.
- **Persistencia**: flag + checklist en `data JSONB` de la cotizacion (`data.decorado` / `data.calcaChecklist`); NO se migro schema. Store gana `actualizarDatos(id, campos)` (merge JSONB `COALESCE(data,'{}')||$2::jsonb` + fallback JSON, espeja prospectos-store).
- **Rutas (server.js, auth dueno-o-admin via cotizacionOperable, igual que /reunion)**: PATCH /decorado (marca/desmarca; marcar activa checklist inicial, conserva el previo si ya existia), PATCH /calca-paso (marca/revierte; paso 6 + archivos -> Dropbox fire-and-forget), POST /liberar (gate).
- **Frontend**: `buildDecoradoControlHtml(o)` en pipeline-logica.js (solo tipo cotizacion; no decorada -> boton "Marcar decorada"; decorada -> "Calca N/6" + checklist togglable + input file en paso 6). Usa `o.refId ?? o.id` (LECCION #57; Q50 con forma prefijada c10). Cableado app.js: marcarDecorada/toggleCalcaPaso/subirCalcaArchivos (leerArchivoBase64 con FileReader). `cotizacionAOportunidad` ahora carga decorado/calcaChecklist; GET /api/cotizaciones los expone desde data. CSS minimo (.decorado-control/.calca-checklist, tokens OKLCH, marca "OK" ASCII).

### Commits (rama issue-61-decorados)
- ad2b781 feat: logica pura del checklist de decorados con gate puedeLiberar (#61) — decorados-logica.js + decorados-logica.test.cjs
- ad81251 feat: el store de cotizaciones mergea campos en data (decorado, checklist) (#61) — cotizaciones-store.js + cotizaciones-store.test.js
- 3dd495b feat: rutas de decorados (marcar, checklist, Dropbox, gate a Pedido liberado) (#61) — server.js + seguimiento-api.test.js
- f7c4551 feat: checklist de calca con progreso en la tarjeta de cotizacion (#61) — pipeline-logica.js, pipeline-logica.test.cjs, app.js, server.js, style.css

### Archivos tocados
- Nuevos: public/js/decorados-logica.js, public/js/__tests__/decorados-logica.test.cjs
- Modificados: lib/cotizaciones-store.js, server.js, public/js/pipeline-logica.js, public/js/app.js, public/css/style.css, test/cotizaciones-store.test.js, test/seguimiento-api.test.js, public/js/__tests__/pipeline-logica.test.cjs

### DEMO (2 min) para Adrian
1. Login. Ir a Pipeline (bottom-nav), tablero. Sobre una cotizacion en Seguimiento aparece el boton **"Marcar decorada (calca)"**.
2. Tocarlo: la tarjeta muestra **"Calca 0/6"** y el checklist de los 6 pasos (cada uno con boton **Marcar**).
3. Marcar pasos: el progreso sube (1/6, 2/6...). Cada paso marcado muestra "OK" y un boton **Revertir**.
4. Intentar liberar con el checklist incompleto (via ruta `POST /api/cotizacion/<id>/liberar`): responde **409** "checklist de calca esta incompleto" (no avanza). Completar los 6 pasos -> el mismo POST responde **200** y marca data.pedidoLiberado. Una cotizacion NO decorada libera siempre (200).
5. Paso 6 (Archivos de posicion de calca subidos a Dropbox): elegir archivos en el input file y "Subir a Dropbox" -> marca el paso y sube (fire-and-forget; en local sin Dropbox configurado igual marca el paso).

### Deuda / pendientes
- El frontend (control de decorado + handlers) no tiene test de DOM (patron del repo: sin DOM); la logica pura si (Q47-Q52) y el cableado debe verificarse en navegador (origen del bug de #57; el orquestador puede capturar via chrome-devtools MCP + npm start).
- El gate `/liberar` es el punto de enforcement MINIMO: hoy ninguna ruta normal lleva una cotizacion a pedido_liberado (eso lo dirige #62 leyendo Operam). #62 DEBE invocar puedeLiberar antes de mover a pedido_liberado. NO se modelo el mapeo estado->etapa post-venta.
- El paso de archivos guarda en `/PELTRE NACIONAL/CALCAS/POSICIONES`; confirmar la ruta real de Dropbox con Adrian si difiere (asumida por analogia con la de CSF).
...
### SLICE AC2 — capa de IO + wiring (rama issue-62-sync-io, 2026-06-17)

> CORRECCION CRITICA del mapeo: el slice del nucleo (arriba) y la investigacion previa codificaron el mapeo MAL (11=pedido, 30=remision, pagos de tipo 13) por las etiquetas erroneas del MCP. La tabla REAL quedo en peltre-operam.md SECCION 12 (FrontAccounting), confirmada en vivo (El Pendulo, order_ 7077): **10=factura** (de aqui salen los montos de pago allocated/outstanding/total), **13=remision**, **30=pedido/Sales Order** (listar_pedidos), 11=nota credito, 12=pago (order_=0). Este slice REMAPEO hechosDesdeOperam a lo real; el nucleo etapaPostVenta (forma de hechos) NO se toco.

**F1 — remapeo hechosDesdeOperam** (`lib/sync-operam.js`): 10=factura->pago, 13=remision->tieneRemision, 30=pedido->tienePedido. Acepta `type` (string) y `trans_type` (numero). Tests reescritos a fixtures reales (test/sync-operam.test.js, 28 tests).

**F2 — lecturas + motor de reconciliacion**:
- `operam-client.js`: `listarTransacciones({rfc,customerId,filterType,desde,hasta})` -> GET /api/v3/sales/transactions; `listarPedidos({debtorNo,desde,hasta})` -> GET /api/v3/sales/sales_orders. Endpoints confirmados contra Postman v3 + API en vivo. Reusan apiCall (Bearer + auto-refresh 401).
- `cotizaciones-store.js`: `cambiarEtapa(id,etapa,evento)` + columnas etapa/eventos (ALTER TABLE IF NOT EXISTS, idempotente). La etapa persistida la respeta migrarCotizacion (no se recalcula del estado). Las etapas post-venta viven en la COTIZACION (la oportunidad que lleva la cotizacion), no en el prospecto.
- `lib/sync-operam-io.js` (NUEVO): `reconciliarOportunidad(op)` lee Operam read-only, normaliza a hechos, aplica etapaPostVenta y mueve la tarjeta via cambiarEtapa. `reconciliarPorIdentificador(ident, ops)` filtra candidatas por RFC y prioriza order_ exacto. `esActivaPostVentaCandidata`. Dependencias inyectables (tests sin Operam real). Respeta gate #61 y monotonia (las decide el nucleo).

**F3 — webhook** `POST /api/webhooks/operam` (server.js): auth por header secreto `X-Operam-Webhook-Secret` (env `OPERAM_WEBHOOK_SECRET`, NO el JWT). Log idempotente en Neon (tabla `operam_webhooks_log`, event_key UNIQUE, graceful sin pool — db.js). `lib/sync-operam-webhook.js`: `extraerIdentificador` (order_/rfc/customer_id defensivo, varias formas/anidamientos) + `claveEvento` idempotente + log. El webhook es SEÑAL: no confia en su payload, la reconciliacion lee la verdad por API. Responde 200 aunque no ligue o Operam este caido.

**F4 — reconciliacion on-demand** `POST /api/sync-operam` (JWT del cotizador): red de seguridad. Recorre SOLO oportunidades activas no terminadas con RFC, mismo motor, best-effort.

**Suite: 698 -> 741 pass / 0 fail** (+43 tests). Archivos nuevos: lib/sync-operam-io.js, lib/sync-operam-webhook.js, test/sync-operam-io.test.js, test/sync-operam-webhook.test.js. Modificados: lib/sync-operam.js, lib/operam-client.js, lib/cotizaciones-store.js, lib/db.js, server.js, test/sync-operam.test.js, test/operam-client.test.js, test/cotizaciones-store.test.js, test/server.test.js, CONTEXT.md.

### VALVULA DE SEGURIDAD — decision de dominio NO confirmada en vivo (para HITL)
**El quote_id de #63 (data.folioOperam, devuelto por POST /api/v3/sales/quote) NO se confirmo en vivo que sea el order_no del Sales Order.** En FrontAccounting una cotizacion/quote puede confirmarse a pedido con un order_no distinto. Por eso el binding por order_ es BEST-EFFORT: si el folio (o data.orderOperam) coincide con un order_ real de la cadena, se filtra a esa cadena; si no, se agrega por cliente (correcto cuando el cliente tiene UNA sola oportunidad activa — el caso comun de Peltre). 
- **Riesgo residual:** un cliente con VARIAS oportunidades activas a la vez SIN order_ resuelto podria mover la etapa equivocada. Mitigacion lista: poblar `data.orderOperam` en la oportunidad da binding preciso sin tocar el motor.
- **Accion HITL recomendada:** capturar en vivo si quote_id == order_no (crear una cotizacion via #63 y comparar con listar_pedidos del cliente). Si coinciden, el binding ya es preciso por folioOperam. Si no, definir como poblar data.orderOperam (p.ej. resolver el order_ al subir a Operam, o leerlo del webhook de Pedido/Nuevo).

### AC4 — N/A (no aplica modo manual)
AC4 ("etapas sin dato expuesto -> modo manual con sugerencia"): NO APLICA. La investigacion confirmo que las 4 etapas post-venta tienen dato expuesto en la API (pagos via factura 10, pedido via Sales Order 30, remision via tipo 13). Decision de Adrian (AC1): las 4 son AUTO. No se construye modo manual.

### Activacion HITL pendiente (fuera de alcance de este slice — la hace el dueno)
1. Configurar en Operam (admin/web_hooks.php) los webhooks salientes: Modelos **Pago de Cliente**, **Pedido**, **Notas de Entrega/Remision** -> Evento **Nuevo** -> URL `<deploy>/api/webhooks/operam` + Header `X-Operam-Webhook-Secret: <secreto>`.
2. Setear `OPERAM_WEBHOOK_SECRET` en Render (env var nueva).
3. Capturar el payload real de un webhook (disparar un evento) para, si hiciera falta, endurecer extraerIdentificador (hoy es defensivo y tolera varias formas).
4. Subir el tier de Render si el free tier duerme y pierde webhooks (la reconciliacion on-demand lo cubre como red de seguridad).

### #62 — estado: AC1 hecho/documentado, AC2 VERDE (este slice), AC3 VERDE, AC4 N/A. Listo para verificacion del orquestador + demo de Adrian; NO cerrado por el subagente.
