...
**Único pendiente post-PRD = ACTIVACIÓN de #62 (HITL, tarea del dueño, no de código):** configurar los webhooks en Operam (`admin/web_hooks.php`: Pago de Cliente / Pedido / Remisión → Nuevo → URL de Render + header `X-Operam-Webhook-Secret`), setear `OPERAM_WEBHOOK_SECRET` en Render, capturar un payload real para endurecer el extractor si hiciera falta, y (opcional) subir Render a Starter para no perder webhooks por cold-start. La reconciliación on-demand (`POST /api/sync-operam`) ya funciona como red de seguridad sin esa activación. **Deuda menor de dominio:** confirmar en vivo si `quote_id` (#63) == `order_no`; si no, poblar `data.orderOperam` para binding preciso (hoy best-effort por cliente, correcto para 1 oportunidad activa por cliente).
