...
**#51 EN CURSO — 3 iteraciones en producción (deploy 372b0d0 live), issue ABIERTO hasta visto bueno de Adrián.** Iteración 3 (feedback de su revisión móvil en prod): import Abastur movido a Admin, botón Cotizar en cards con prefill, lista con cards compactas (fix desbordes), "Registrar contacto" (ex Toque), Ganada/Perdida como botones en tablero de cotizaciones (touch sin drag), WhatsApp prefiere celEntrega, mensaje de seguimiento personalizado sin emoji (se rompía en el draft), toast explicando "Hecho". **DECISIÓN PENDIENTE de Adrián: "pre-cotizaciones"** — propuso renombrar las cotizaciones sin registro en Operam; recomendé atributo/badge ("Operam #N" vs "Sin registrar") en vez de renombrar la entidad — esperar su respuesta; si insiste, grilling + renombre de glosario. Hecho: tokens OKLCH (lienzo azul pizarra desaturado tras feedback; cobalto en acciones), pills de etapa con color+contador, sumas $ por columna en cotizaciones, quick-add, tarjetas compactas (avance visible — en touch no hay drag, "Más" guarda lo pesado), columnas válidas iluminadas durante arrastre, estados vacíos punteados, chips tinte claro+texto oscuro (AA sol), tipografía sistema. Suite 479/479. Referencias de Bitrix del usuario en .temporales\Screenshot_20260611_*.jpg (¡incluyen sus etapas reales de Bitrix para el mapeo de #48!). PRODUCT.md existe en el repo; DESIGN.md no (correr /impeccable document tras cerrar #51).
