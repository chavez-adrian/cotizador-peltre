...
- **#62 AC2 VERDE** 🟢 (ABIERTO, rama `issue-62-sync-io`, sin merge) — núcleo puro (AC3) + **capa de IO + wiring (AC2)**: remapeo de `hechosDesdeOperam` al mapeo REAL (peltre-operam.md §12: 10=factura/pago, 13=remisión, 30=pedido — NO las etiquetas del MCP); lecturas `listarTransacciones`/`listarPedidos` (operam-client); motor `reconciliarOportunidad` (`lib/sync-operam-io.js`); webhook `POST /api/webhooks/operam` (header secreto + log idempotente Neon) + reconciliación on-demand `POST /api/sync-operam`. Suite 741/741. AC1 documentado, AC4 N/A (4 etapas AUTO). **Pendiente HITL (dueño):** configurar webhooks en Operam + `OPERAM_WEBHOOK_SECRET` en Render + capturar payload real. **Válvula:** quote_id↔order_no no confirmado en vivo (binding best-effort por cliente; ver sección #62). NO cerrado por el subagente.
