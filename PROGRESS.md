...
- **#68** Subir a Operam — crítico **MERGEADO parcial**. AC1/AC2 HECHOS (cliente correcto, ya no usa `clientes[0]`; 422 si no identifica). AC3 parcial:
  ...
  - **línea de envío como partida: BLOQUEADA (decisión de dominio).** Hoy el envío va en `comments`, NO como partida. Verificación read-only en vivo (2026-06-18) reveló que el mapeo método→SKU NO es inequívoco y el mapeo previo estaba incompleto. Catálogo REAL de fletes en Operam (category 4 Servicios, cuentas 401-01-001/601-32-001): `250911001` "Envio Foraneo" · `251021001` "Envio Económico FedEx Ground®" (CDMX) · **`251021002` "Envio Económico FedEx Ground® Foraneo"** (NO estaba en el mapeo del prompt) · `220906001` Lalamove camioneta chica (500kg) · `220906002` Lalamove camioneta grande (1000kg) · `220906003` Lalamove auto (200kg) · `FLETELOCALCDMX` desuso. **Ambigüedades sin resolver (requieren a Adrián):** (1) Lalamove se elige por TAMAÑO DE VEHÍCULO, dato que el cotizador no captura (el item ENVIO solo trae texto+precio); (2) FedEx tiene SKU CDMX y Foráneo separados, hay que cruzar carrier+zona pero el cotizador no marca CDMX/foráneo explícito; (3) DHL/UPS/Estafeta (carriers que el cotizador ofrece via envia.com) no tienen SKU — ¿van a `250911001` genérico o solo aplica a FedEx? No hay SKU "neutro" seguro: un SKU mal elegido imputa mal el ingreso/COGS de flete (riesgo contable real). NO se adivinó.
