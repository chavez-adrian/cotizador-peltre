...
**#51 EN CURSO (rediseño UI Bitrix con /impeccable, 2 iteraciones mergeadas a producción para revisión de Adrián desde su teléfono — issue ABIERTO hasta su visto bueno final).** Hecho: tokens OKLCH (lienzo azul pizarra desaturado tras feedback; cobalto en acciones), pills de etapa con color+contador, sumas $ por columna en cotizaciones, quick-add, tarjetas compactas (avance visible — en touch no hay drag, "Más" guarda lo pesado), columnas válidas iluminadas durante arrastre, estados vacíos punteados, chips tinte claro+texto oscuro (AA sol), tipografía sistema. Suite 479/479. Referencias de Bitrix del usuario en .temporales\Screenshot_20260611_*.jpg (¡incluyen sus etapas reales de Bitrix para el mapeo de #48!). PRODUCT.md existe en el repo; DESIGN.md no (correr /impeccable document tras cerrar #51).
