...
- **#67** Espejo de folios + binding por `trans_no_from` (verificado: pedido 7269 ← cotización 1141). **AC2-AC5 HECHOS** (rama issue-67, 803 tests verde): AC2 `resolverOrderDeOportunidad` resuelve el order_ por prioridad explicito>`trans_no_from===folioOperam`>cliente; venta directa (trans_no_from vacio) no liga. AC3 `construirEspejoOperam`+`setEspejoOperam` persisten `data.espejoOperam={cotizacion,pedido,factura:{numero,ref},remisiones,pagos,notasCredito}` solo con binding preciso (reconciliarOportunidad lo guarda aunque la etapa no cambie). AC4 `cadenaOperamTexto`/`cadenaOperamHtml` (pipeline-logica) pintan "Cot #N - Pedido #N - Factura A1907 - Remision - Pagado" en la tarjeta (tablero+lista); server expone espejoOperam en /api/cotizaciones. AC5 tests de cada AC (resolucion por documento, persistencia, venta directa no liga, cadena de la tarjeta). **AC1 (cruce en vivo de una cotizacion-del-cotizador -> pedido, confirmar trans_no_from==folioOperam) queda para el orquestador/HITL: exige WRITES en Operam.** Asumido (validar contra API real): pedido trae `order_no`+`trans_no_from`+`debtor_no`; transacciones traen `order_`+`trans_no`+`ref`+`type/trans_type` (10 factura, 13 remision, 12 pago, 11 NC).
